...
ggsave(output_file, shapeplot_RA_incident_step_tertilesHealthy_sex_ethnicity_qualif_season_wear_smoking_alcohol_processed_meat_fresh_fruit_oily_fish, device = "tiff", width = 6, height = 8, dpi = 300)
```
# Spline plots
...
tertile1 <- quantile(datHealthy$med_steps, 1/3)
step_ref <-  mean(datHealthy$med_steps[datHealthy$med_steps < tertile1]) #5112.999 use mean steps in lowest category so match 
...
ggsave(output_file, shapeplot_RA_incident_step_tertilesOverweight_sex_ethnicity_qualif_season_wear_smoking_alcohol_processed_meat_fresh_fruit_oily_fish, device = "tiff", width = 6, height = 8, dpi = 300)
```
...
tertile1 <- quantile(datOverweight$med_steps, 1/3)
step_ref <-  mean(datOverweight$med_steps[datOverweight$med_steps < tertile1]) #5112.999 use mean steps in lowest category so match 
spl_ref <- predict(spl, step_ref) # spline terms at reference value of variable
spl_all <- predict(spl, steps) # spline terms across steps
L <- t(spl_all) - c(spl_ref) # matrix of spline terms, centred for reference value of variable
...
ggsave(output_file, shapeplot_RA_incident_step_tertilesObese_sex_ethnicity_qualif_season_wear_smoking_alcohol_processed_meat_fresh_fruit_oily_fish, device = "tiff", width = 6, height = 8, dpi = 300)
```
...
tertile1 <- quantile(datObese$med_steps, 1/3)
step_ref <-  mean(datObese$med_steps[datObese$med_steps < tertile1]) #5112.999 use mean steps in lowest category so match 
spl_ref <- predict(spl, step_ref) # spline terms at reference value of variable
spl_all <- predict(spl, steps) # spline terms across steps
L <- t(spl_all) - c(spl_ref) # matrix of spline terms, centred for reference value of variable
