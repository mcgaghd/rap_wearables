...
dir.create("plots")
output_dir <- "plots"
output_file <- file.path(output_dir, "PsA_quartiles.tiff")
ggsave(output_file, shapeplot_PsATotal_incident_step_quartiles_sex_tdi_quarters_qualif_smoking_alcohol, device = "tiff", width = 6, height = 8, dpi = 300)
```
...
# CYCLE OVER ADJUSTMENTS=========================================
 for (adjustment in adjustments) {
  ...
  textsize <- 5.5
  ext <- c(0, 0)
  lim_rad_scale <- 1/min(fp_frame_step_quartiles_PsATotal_incident$stderr) + 1
  ybreaks <- c(0.2, 0.5, 1,1.5, 2)  # Include 2 in the breaks
  ylim_scale <- 1
...
# Calculate the limits for the y-axis
 #ylims <- c(ybreaks[1] - 0.02, ybreaks[length(ybreaks)] + 0.01)
  ...
  ruglinewidth <- 0.03
  datatoplot <- fp_frame_step_quartiles_PsATotal_incident
...
# CYCLE OVER ADJUSTMENTS=========================================
 for (adjustment in adjustments) {
  ...
  textsize <- 5.5
  ext <- c(0, 0)
  lim_rad_scale <- 1/min(fp_frame_step_quintiles_PsATotal_incident$stderr) + 1
  ybreaks <- c(0.2, 0.5, 1,1.5, 2)  # Include 2 in the breaks
  ylim_scale <- 1
...
# Calculate the limits for the y-axis
 #ylims <- c(ybreaks[1] - 0.02, ybreaks[length(ybreaks)] + 0.01)
  ...
  ruglinewidth <- 0.03
  datatoplot <- fp_frame_step_quintiles_PsATotal_incident
...
shapeplot1 <- ggplot(datatoplot, aes(x = `mean_steps`, y = exp(estimate))) +
 ...
 # Add rug plot
  ...
  assign(paste0("shapeplot_PsATotal_incident_step_quintiles_", gsub(",", "_", adjustment)), shapeplot1)
   }
```
...
dir.create("plots")
output_dir <- "plots"
output_file <- file.path(output_dir, "PsA_quintiles.tiff")
ggsave(output_file, shapeplot_PsATotal_incident_step_quintiles_sex_tdi_quarters_qualif_smoking_alcohol, device = "tiff", width = 6, height = 8, dpi = 300) 
```
