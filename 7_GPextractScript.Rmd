...
psori_table <- dat_GPmap %>%
  filter(grepl("insomnia", TERMV3_DESC, ignore.case = TRUE))
```
...
```{r}
somnolence_Strings<- "(^1B6C.|Fy01.|R0001|XaZKa|Fy01.|R0001|YM0vE|YMAyr|YavSB)"

# Create the new column "PsO_v2" based on the matching condition
somnolence_Read2 <- dat_GPevent %>%
  filter(grepl(somnolence_Strings, read_2, ignore.case = TRUE))

somnolence_Read3 <- dat_GPevent %>%
  filter(grepl(somnolence_Strings, read_3, ignore.case = TRUE))
