...
```{r}
write.csv(dat, "prepped_steps_before_exclude_1.csv", row.names = FALSE)
write.csv(dat_GP, "prepped_steps_before_exclude_GP_1.csv", row.names = FALSE)
```
...
for (
  ...
  nb <- nrow(dat_GP)
  print(cov)
  missing_cov <- is.na(dat_GP[, cov])|(as.character(dat_GP[, cov]) == "") |(as.character(dat_GP[, cov]) == "Missing") # for safety coerce to character for second check as can return NA on some classes e.g. Date
  dat_GP <- dat_GP[!missing_cov,]
  tab_exc <- rbind(
    tab_exc,
    data.frame(
      "Exclusion" = paste0("Missing ", cov),
      "Number_excluded" = nb - nrow(dat_GP),
      "Number_remaining" = nrow(dat_GP)
    )
  )
}
```
...
```{r}
nb <- nrow(dat_GP)
dat_GP <- dat_GP[dat_GP$total_prevalent_IA == FALSE, ] 
...
subset for prevalent PsO in each
...
write.csv(datPsO_GP, "self_report_PsA_GP.csv")
```
...
minimal_1 <- coxph(Surv(age_entry_days, age_exit_days_1, PsA_incident_HES) ~ 
                   as.factor(MET_quartiles), data = datPsO)
...
model_BMI <- coxph(Surv(age_entry_days, age_exit_days_1, PsA_incident_HES) ~ 
                   as.factor(MET_quartiles) + as.factor(qualif) + as.factor(smoking) + 
                   as.factor(tdi_quarters) + as.factor(alcohol)+as.factor(BMI_cats), data = datPsO)
...
model_SystemicTreatments <- coxph(Surv(age_entry_days, age_exit_days_1, PsA_incident_HES) ~ 
                   as.factor(MET_quartiles) + as.factor(qualif) + as.factor(smoking) + 
                   as.factor(tdi_quarters) + as.factor(alcohol)+as.factor(SystemicTreatment), data = datPsO)
