...
dx download /users/mcgaghd/PrimaryCare/PrimaryCareClean/datMethotrexate_gp.csv -r
...
dat_MTXmonitoring_gp <- fread("datMethotrexate_gp.csv", data.table = FALSE) # fread is a function
```
...
Only run this for the PPV stats for case definition
```{r}
#dat$date_baseline <- NULL 

#For PsA
#dat$date_baseline <- as.Date("2016-05-31")

#Re-run for probables 
#dat$date_baseline <- as.Date("2014-05-31")
```
...
dat <- merge(
  ...
  hes_disease_lookup(dat_hes, dat, ProbPsA_icd10_codes, ProbPsA_icd9_codes,
                     "prevalent", "hes_prevalent_ProbPsA"),
  by = "eid",
  all.x = TRUE,
  suffixes = c("", "dup")
)
...
dat$ProbPsA_prevalent <- ((dat$hes_prevalent_ProbPsA & dat$total_prevalent_PsO))  #(!dat$total_prevalent_IA)

sum(dat$ProbPsA_prevalent)
...
dat_venn$total_Def_PsA <- (dat_venn$self_reported_PsA | dat_venn$PrevalentPsA_gp | dat_venn$hes_prevalent_PsA)

dat_venn$'Definite PsA' <- dat_venn$total_Def_PsA
dat_venn$'Probable PsA HES' <- dat_venn$ProbPsA_prevalent
dat_venn$'Probable PsA GP' <- (dat_venn$PrevalentProbPsA_gp  & dat_venn$total_prevalent_PsO)

plot <- ggvenn(dat_venn, c('Definite PsA', 'Probable PsA GP'),
...
dat_venn$'Definite PsA HES' <- dat_venn$hes_prevalent_PsA
dat_venn$'Probable PsA HES' <- dat_venn$ProbPsA_prevalent
dat_venn$'Definite PsA GP' <- dat_venn$PrevalentPsA_gp
dat_venn$'Self-report PsA' <- dat_venn$self_reported_PsA


plot <- ggvenn(dat_venn, c('Definite PsA HES', 'Probable PsA HES', 'Definite PsA GP', 'Self-report PsA'),
               fill_color = c("#0073C2FF", "#EFC000FF", "#CD534CFF", "#7AA974FF"),
  stroke_size = 0.5, set_name_size = 4
  )
plot
...
dat <- merge(
  ...
  hes_disease_lookup(dat_hes, dat, SeropositiveRA_icd10_codes,SeropositiveRA_icd9_codes,
                     "prevalent", "hes_prevalent_seropositiveRA"),
  by = "eid",
  all.x = TRUE,
  suffixes = c("", "dup")
)
...
dat <- dat %>%
  ...
  mutate(MTXmonitoring_gp = !is.na(event_dt_MTXmonitor) & !is.na(date_baseline) & event_dt_MTXmonitor < date_baseline)
```
...
###Total indicators 
```{r}
dat$MTX_any <- (dat$GP_MTX |dat$methotrexate| dat$MTXmonitoring_gp) 
...
```{r}
write.csv(dat, "self_report_PsA_final_wholeCohort.csv")
```

```{bash}
dx upload self_report_PsA_final_wholeCohort.csv --dest users/mcgaghd/PsAfinal/FinalCohorts/SelfReport/self_report_PsA_final.csv
```
...
##Uploaded to  PsAFinal 
## Write out the whole cohort for collider stratification bias analysis 
```{r}
write.csv(dat, "self_report_PsA_final_PsO.csv")
```

```{bash}
dx upload self_report_PsA_final_PsO.csv --dest users/mcgaghd/PsAfinal/FinalCohorts/SelfReport/self_report_PsA_final_PsO.csv
```
...
dat_venn$'Definite PsA HES' <- dat_venn$PsA_incident_HES
dat_venn$'Probable PsA HES' <- dat_venn$ProbPsA_incident
dat_venn$'Definite PsA GP' <- dat_venn$PsA_incident_gp


plot <- ggvenn(dat_venn, c('Definite PsA HES', 'Probable PsA HES', 'Definite PsA GP'),
               fill_color = c("#0073C2FF", "#EFC000FF", "#CD534CFF"),
  stroke_size = 0.5, set_name_size = 4
  )
plot
...
dat_venn_GP$ProbDefPsA_incident_date <- as.Date(dat_venn_GP$ProbDefPsA_incident_date, format = "%Y-%m-%d")

# Filter the data to remove rows where ProbDefPsA_incident_date is after 01/01/2016

dat_venn_GP <- dat_venn_GP %>%
  filter(is.na(ProbDefPsA_incident_date) | ProbDefPsA_incident_date <= as.Date("2017-01-01"))


dat_venn_GP$'Definite PsA HES' <- dat_venn_GP$PsA_incident_HES
dat_venn_GP$'Probable PsA HES' <- dat_venn_GP$ProbPsA_incident
dat_venn_GP$'Definite PsA GP' <- dat_venn_GP$PsA_incident_gp


plot <- ggvenn(dat_venn_GP, c('Definite PsA HES', 'Probable PsA HES', 'Definite PsA GP'),
               fill_color = c("#0073C2FF", "#EFC000FF", "#CD534CFF"),
  stroke_size = 0.5, set_name_size = 4
  )
plot
...
In terminal, run the following code: `dx upload self_report_PsA_final.csv --dest users/mcgaghd/PsAfinal/self_report_PsA_final.csv`
