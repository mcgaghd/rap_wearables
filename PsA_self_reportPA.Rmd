...
```{r}
dat$PsA_incident_HES_GP <- dat$PsA_incident_gp | dat$PsA_incident
```

Take the first date in either GP date or incident date

```{r}
dat$PsA_incident_HES_GP_date <- pmin(dat$PsA_incident_date, dat$Incident_PsA_gp_date, na.rm = TRUE)
...
1. Definite HES PsA (can use full follow up but for GP record censoring dates) - 'PsA_incident'
2. Definite total PsA (GP and HES) (use GP censoring dates) - 'PsA_incident_HES_GP'
3. Probable total PsA (using HES records) ' ProbDefPsA_incident'
4. Probable PsA only -'ProbPsA_incident'

```{r}
# Define follow-up dates for each outcome
dat$date_fu_1 <- dat$date_fu_2 <- dat$date_fu_3 <- dat$date_fu_4 <- dat$date_cens

# Outcome 1: Definite HES PsA (can use full follow-up but for GP record censoring dates)
dat$date_fu_1[dat$PsA_incident] <- dat$PsA_incident_date[dat$PsA_incident]

# Outcome 2: Definite total PsA (GP and HES) (use GP censoring dates)
dat$date_fu_2[dat$PsA_incident_HES_GP] <- dat$PsA_incident_HES_GP_date[dat$PsA_incident_HES_GP]

# Outcome 3: Probable total PsA (using HES records)
dat$date_fu_3[dat$ProbDefPsA_incident] <- dat$ProbDefPsA_incident_date[dat$ProbDefPsA_incident]

# Outcome 4: Probable PsA only
dat$date_fu_4[dat$ProbPsA_incident] <- dat$ProbPsA_incident_date[dat$ProbPsA_incident]
...
dat$fu_time_4 <- as.numeric(difftime(dat$date_fu_4, dat$date_baseline, units = "days"))
...
dat$age_exit_days_4 <- dat$age_entry_days + dat$fu_time_4
...
dat$age_exit_days2_4 <- as.numeric(difftime(dat$date_fu_4, dat$approx_dob, units = "days"))
...
if (!isTRUE(all.equal(dat$age_exit_days_4, dat$age_exit_days2_4))) {
  stop("Different methods of calculating age at exit give different answers for Outcome 4")
}
...
datPsO$MET_quartiles <- qtile_cut(datPsO$summed_MET_minutes, probs = seq(0, 1, by = 1/4), dp_label = 1)

datPsO <- datPsO %>%
  mutate(MET_quartiles = factor(MET_quartiles,
                                levels = c("<758.0", "758.0-1,781.9", "1,782.0-3,545.7", "3,545.8+")))
```

Modelling across the four outcomes 

```{r}
# Model 1: Definite HES PsA
model_1 <- coxph(Surv(age_entry_days, age_exit_days_1, PsA_incident) ~ 
                   as.factor(MET_quartiles) + as.factor(qualif) + as.factor(smoking) + 
                   as.factor(tdi_quarters) + as.factor(alcohol), data = datPsO)

summary(model_1)

# Model 2: Definite total PsA (GP and HES)
model_2 <- coxph(Surv(age_entry_days, age_exit_days_2, PsA_incident_HES_GP) ~ 
                   as.factor(MET_quartiles) + as.factor(qualif) + as.factor(smoking) + 
                   as.factor(tdi_quarters) + as.factor(alcohol), data = datPsO)
