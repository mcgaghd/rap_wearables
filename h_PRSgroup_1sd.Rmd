...
```{r}
final_df <- final_df %>%
  mutate(Heterogeneity_Label = sprintf("Heterogeneity: X^2_(%d) = %.2f (p=%.3f)", df, x_chi, p_value))
...
last_y_position <- max(final_df$mo, na.rm = TRUE)

heterogeneity_text_plot <- ggplot() +
  annotate("text", x = 0, y = last_y_position + 1, label = heterogeneity_label, hjust = 0, size = 3) +
  theme_void() +
