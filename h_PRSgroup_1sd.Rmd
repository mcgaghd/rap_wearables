...
Restandardise PRS groups to cohort 
...
```{r}
dat$PRS_RA_RiskGroup <- factor(dat$PRS_RA_RiskGroup, levels = c("Low", "Intermediate", "High"))

contrasts(dat$PRS_RA_RiskGroup) <- contr.treatment(n = length(levels(dat$PRS_RA_RiskGroup)))
...
# Create datInter for PRS_RA_RiskGroup == "Intermediate"
datInter <- dat %>%
  filter(PRS_RA_RiskGroup == "Intermediate")

# Create datHigh for PRS_RA_RiskGroup == "High"
datHigh <- dat %>%
...
```{r}
coxLow <- coxph(Surv(age_entry_days, age_exit_days, RA_incident) ~ 
...
coxInter <- coxph(Surv(age_entry_days, age_exit_days, RA_incident) ~ 
                             med_steps_std + strata(sex) +
                             as.factor(tdi_quarters) + as.factor(qualif) +
                             as.factor(smoking) + as.factor(alcohol) + as.factor(red_processed_total) + as.factor(fruit_veg_total),
                             data = datInter)

coxHigh <- coxph(Surv(age_entry_days, age_exit_days, RA_incident) ~ 
                             med_steps_std + strata(sex) +
                             as.factor(tdi_quarters) + as.factor(qualif) +
                             as.factor(smoking) + as.factor(alcohol) + as.factor(red_processed_total) + as.factor(fruit_veg_total),
                             data = datHigh)
...
for(i in 1:3) {
    # Fit Cox model for the current PRS group
    cox_models[[i]] <- coxph(Surv(age_entry_days, age_exit_days, RA_incident) ~ 
                             med_steps_std*interaction(PRS_group_numeric == i) + strata(sex) +
                             as.factor(tdi_quarters) + as.factor(qualif) +
                             as.factor(smoking) + as.factor(alcohol) + as.factor(red_processed_total) + as.factor(fruit_veg_total),
                             data = dat)
}
...
sub_PRS <- lincom(cox_PRS, c("med_steps_std",
                          "med_steps_std:PRS_RA_RiskGroup.L",
                          "med_steps_std:PRS_RA_RiskGroup.Q"),
       eform=TRUE)
res <- as.data.frame(sub_PRS)
...
final_df$group <- factor(final_df$group, levels = c("Sex","Age","BMI",  "Overall"))
...
final_df <- final_df %>%
  mutate(
    heterogeneity = sprintf("(italic(chi^2)[%d] == %.2f * ';' ~ italic(p) == %.2f)", 
                            as.numeric(df), as.numeric(x_chi), as.numeric(p_value))
  )
...
forest_plot <- ggplot(final_df, aes(x = Estimate, y = model, xmin = `Lower CI`, xmax = `Upper CI`)) +
  geom_point(size = 2) +
  geom_errorbarh(aes(height = 0)) +
    geom_point(data = subset(final_df, model != "Overall"), aes(x = Estimate, y = model, size = 0.7/std_error), shape = 15, colour = "black", fill = "black", stroke = 0.5) +
  geom_point(data = subset(final_df, model == "Overall"), aes(x = Estimate, y = model, size = 0.7/std_error), shape = 18, colour = "black", fill = "black", stroke = 0.5) +
  ...
  theme(
    ...
    panel.spacing = unit(2, "lines"),
    panel.spacing.y = unit(2, "lines"),
    plot.margin = margin(7, 3,10, 3, "points")) +
  ylab(NULL) +
  coord_cartesian(clip = "off") 
  #coord_cartesian(xlim = c(min(xbreaks), max(xbreaks)), clip = 'off')# Adjust right margin to create space for text
...
forest_plot <- forest_plot + 
  ...
  #geom_text(aes(y = model, x = 1.35, label = paste0(HR_Text)), 
   #         fontface = ifelse(model =="Overall", "bold", "plain")),
    #        hjust = -0.2, vjust = 0.55, size = 3, color = "black") +
   geom_text(data = final_df, aes(y = model, x = 1.35, label = HR_Text, fontface = ifelse(model == "Overall", "bold", "plain")), 
            hjust = -0.2, vjust = 0.55, size = 3, color = "black") +
  ##HR column - label bolded at top 
  ...
  geom_text(aes(y = model, x = 1.35, 
                label = ifelse(model %in% c("30+", "70-79", "Female"), heterogeneity, NA)),
            hjust = 0, vjust = 1.55, size = 3, color = "black",
            parse = TRUE, check_overlap = TRUE) 
...
```{r}
overall_data <- subset(final_df, model == "Overall")

# Subset for all other groups
other_data <- subset(final_df, model != "Overall")

# Building the plot
forest_plot <- ggplot() +
  geom_errorbarh(data = final_df, aes(xmin = `Lower CI`, xmax = `Upper CI`, y = model), height = 0) +
  geom_point(data = other_data, aes(x = Estimate, y = model, size = 0.7/std_error), shape = 15, colour = "black", fill = "black", stroke = 0.5) +
  geom_point(data = overall_data, aes(x = Estimate, y = model, size = 0.7/std_error), shape = 18, colour = "black", fill = "black", stroke = 0.5) +
  facet_grid(group ~ ., scales = "free", space = "free") +
  scale_radius(guide = "none", limits = c(0, lim_rad_scale), range = c(2, 6)) +
  labs(x = "Hazard Ratio (95% CI) per \n1000 steps increase", y = "") +
  scale_x_continuous(trans = "log", breaks = xbreaks * xlim_scale, limits = visible_xlims) +
