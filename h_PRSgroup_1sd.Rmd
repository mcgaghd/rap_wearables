...
forest_plot <- ggplot(final_df, aes(x = Estimate, y = model, xmin = `Lower CI`, xmax = `Upper CI`)) +
  ...
  theme_minimal() +
  theme(
    strip.text.x = element_blank(),  # Hide the facet labels
...
hr_text_plot <- ggplot(final_df, aes(x = 0, y = model)) +
  ...
  theme(
    ...
    panel.spacing = unit(0, "lines"),  # Remove spacing between facets if necessary
...
# Print the HR text plot for verification
print(hr_text_plot)
```
