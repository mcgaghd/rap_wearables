...
xbreaks <- c(0.01,0.5, 0.75, 1, 1.5)  # Include 2 in the breaks
xlim_scale <- 1
...
forest_plot <- ggplot(final_df, aes(x = Estimate, y = model, xmin = `Lower CI`, xmax = `Upper CI`)) +
  ...
  theme(
    ...
    strip.text.y = element_blank(),
    panel.spacing.y = unit(2, "lines"),
    plot.margin = margin(0, 10, 0, 10, "points")) +
  ylab(NULL) 
...
forest_plot <- forest_plot + 
  geom_text(aes(y = model, x = 1.75, label = paste0(HR_Text)), 
            hjust = -0.2, vjust = 0, size = 3, color = "black") +
  geom_text(aes(y = model, x = 0.5, label = paste0(events)),
            hjust = 0.75, vjust = 0, size = 3, color = "black")
