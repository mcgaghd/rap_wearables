...
final_df <- final_df %>%
  rowwise() %>%
  mutate(Heterogeneity_Label = bquote(Heterogeneity:~X^2[.(df)] == .(x_chi) ~ (italic(p) == .(p_value)))) %>%
  ungroup()

# Use this label in your ggplot2 code with `parse = TRUE`
heterogeneity_text_plot <- ggplot(final_df, aes(x = 0, y = last_y_position)) +
  geom_text(aes(label = as.character(Heterogeneity_Label)), parse = TRUE, hjust = 0, size = 3) +
  theme_void() +
  coord_cartesian(clip = 'off')


final_df$Heterogeneity_Label <- lapply(final_df$Heterogeneity_Label, function(x) parse(text=x))
# Use this label in your heterogeneity text plot code
# Extract the unique heterogeneity label (assuming it's the same for all rows or you're only using the first one)
...
last_y_positions <- final_df %>%
  group_by(group) %>%
  summarize(last_y_position = max(y_position, na.rm = TRUE)) %>%
  ungroup()
