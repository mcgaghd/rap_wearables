...
hr_text_plot <- ggplot(final_df, aes(x = 0, y = model)) +
  ...
  geom_text(aes(label = HR_Text), hjust = 0, size = 3) +# Add the text annotations
...
#  geom_text(data = subset(final_df, y_position == min(y_position)),  # Use only the first row for label
          #  aes(label = "HR (95% CI)"), hjust = -.05, size = 3, vjust = -3) + 
  #geom_text(aes(y = (y_position == 1)),
   #             label = "HR (95% CI)",
    #        vjust = 0,
     #       hjust = 0,
      #      colour = "black") +
...
```
```{r}
# Create a data frame for heterogeneity information

final_df <- final_df %>%
  rowwise() %>%
  dplyr::mutate(Heterogeneity_Label = sprintf("Heterogeneity: X^2[%d] == %.2f ~(italic(p) == %.3f)", df, x_chi, p_value)) %>%
  ungroup()
...
heterogeneity_info_df <- data.frame(
  y_position = c(3, 5, 9),  # Specify the y positions
  Heterogeneity_Label = c(  # Generate the labels for each position
    sprintf("Heterogeneity: X^2[%d] == %.2f ~(italic(p) == %.3f)", df[3], x_chi[3], p_value[3]),
    sprintf("Heterogeneity: X^2[%d] == %.2f ~(italic(p) == %.3f)", df[5], x_chi[5], p_value[5]),
    sprintf("Heterogeneity: X^2[%d] == %.2f ~(italic(p) == %.3f)", df[9], x_chi[9], p_value[9])
  )
)

# Convert labels to character expressions for ggplot to parse
heterogeneity_info_df$Heterogeneity_Label <- as.character(heterogeneity_info_df$Heterogeneity_Label)

# Now create the plot
heterogeneity_text_plot <- ggplot(heterogeneity_info_df, aes(x = 0, y = y_position)) +
  geom_text(aes(label = Heterogeneity_Label), parse = TRUE, hjust = 0, size = 3) +
  theme_void() +
  coord_cartesian(clip = 'off')

# Print the plot to check it
print(heterogeneity_text_plot)
...
final_df <- final_df %>%
  rowwise() %>%
  mutate(Heterogeneity_Label = sprintf("Heterogeneity: X^2[%d] == %.2f ~(italic(p) == %.3f)", df, x_chi, p_value)) %>%
  ungroup()


# Make sure the label is character type
final_df$Heterogeneity_Label <- as.character(final_df$Heterogeneity_Label)


# Use the label in your ggplot2 code with `parse = TRUE`
heterogeneity_text_plot <- ggplot(final_df, aes(x = 0, y = last_y_position)) +
  geom_text(aes(label = Heterogeneity_Label), parse = TRUE, hjust = 0, size = 3) +
  theme_void() +
  coord_cartesian(clip = 'off')
...
# Extract the unique heterogeneity label (assuming it's the same for all rows)
heterogeneity_label <- unique(final_df$Heterogeneity_Label)



# Merge the last positions back into the original dataframe
heterogeneity_text_plot <- ggplot(final_df, aes(x = 0, y = last_y_position)) +
  geom_text(aes(label = Heterogeneity_Label), parse = TRUE, hjust = 0, size = 3) +
  theme_void() +
  coord_cartesian(clip = 'off') 
...
combined_plot <- forest_plot + hr_text_plot +
  plot_layout(widths = c(2.8,1,1))  
combined_plot <- forest_plot + hr_text_plot + heterogeneity_text_plot
