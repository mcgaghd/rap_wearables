...
final_df <- final_df %>%
  mutate(
    model = case_when(
      group == "BMI (categories)" ~ fct_relevel(model, "<25", "25-29.9", "30+"),
      group == "Age (categories)" ~ fct_relevel(model, "40-49", "50-59", "60-69", "70-79"),
      TRUE ~ model  # Keep the order for other groups
    )
  )
...
ggsave(output_file, forest_plot, device = "tiff", width = 11, height = 7, dpi = 300)
```
