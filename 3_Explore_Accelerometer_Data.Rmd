...
# Perform matching with a caliper for age_entry_years
matched_data <- matchit(PsO_prevalent_coded ~ age_entry_years + season_wear + sex, data = dat, method = "nearest", caliper = caliper,ratio = 4, replace=FALSE)#
...
datMatchPsO <- datMatchPsO %>%
  mutate(PsO_prevalent_coded = ifelse(PsO_prevalent_coded, "Psoriasis", "Control"))
...
dat_hour <- datMatchPsO[, c(2, start_loc:end_loc)]
names(dat_hour)[-1] <- hours
...
subset_datMatchPsO <- datMatchPsO %>%
  select(eid, PsO_prevalent_coded)  

#Now join up dat_hour and subset by eid
dat_hour <- dat_hour %>%
  left_join(subset_datMatchPsO, by = "eid") %>%
  group_by(eid) %>%
  mutate(PsO_prevalent_coded = first(PsO_prevalent_coded))

dat_hour_means <- dat_hour %>% 
  group_by(variable, PsO_prevalent_coded) %>% 
  summarise_at(vars(value), funs(mean(., na.rm = TRUE)))

PsO_timing <- ggplot(dat_hour_means, aes(x = variable, y = value, color =
                                           PsO_prevalent_coded)) +  theme_bw() + 
  ggtitle("Acceleration by Time of Day") +
  theme(plot.title = element_text(hjust = 0.5)) +
  geom_line(aes(group=PsO_prevalent_coded)) +
  geom_point() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1), 
        text = element_text(size = 10)) +
  xlab("Hour of Day") + 
