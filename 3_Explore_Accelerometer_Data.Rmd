...
# set up tick marks
#sandbox
datMatchPsAbackup <- datMatchPsA

datMatchPsA <- datMatchPsA %>%
  mutate(total_prevalent_PsA = ifelse(total_prevalent_PsA, "PsA", "Control"))
...
# Group by function for dataframe in R using pipe operator 
dat_hour_means <- dat_hour %>% group_by(total_prevalent_PsA) %>% summarise_at(vars(value),funs(mean(.,na.rm=TRUE)))

dat_hour <- dat_hour %>%
  left_join(datMatchPsA %>% select(eid, total_prevalent_PsA), by = "eid") %>%
  group_by(eid) %>%
  mutate(total_prevalent_PsA = max(total_prevalent_PsA, na.rm = TRUE))
