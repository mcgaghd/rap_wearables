...
# Group by function for dataframe in R using pipe operator 
dat_hour_means <- dat_hour %>% group_by(variable, AnalysisLabel, sex) %>% summarise_at(vars(value),funs(mean(.,na.rm=TRUE)))

p<-ggplot(dat_hour_means[dat_hour_means$sex=='Female',], aes(x=variable, y=value, color=AnalysisLabel)) +
  theme_bw() + 
