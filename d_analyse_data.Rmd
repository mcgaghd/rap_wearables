...
```{r}
# Round up 'med_steps' to the nearest 1000
dat$rounded_steps <- ceiling(dat$med_steps / 1000) * 1000

# Standardize 'rounded_steps' per 1000 steps
dat$standardized_steps <- dat$rounded_steps / 1000
hist(dat$standardized_steps)
```

```{r}
cox1000 <- coxph((Surv(age_entry_days, age_exit_days, RA_incident)) ~ standardized_steps
                           + strata(sex)+
                           as.factor(ethnicity) + as.factor(tdi_quarters) +as.factor(qualif)+
                           as.factor(season_wear) + as.factor(smoking) + as.factor(alcohol)+as.factor(red_processed_total) + as.factor(fruit_veg_total),
                         data=dat)
summary(cox1000)
```
