...
```{r}
exposures <- c("step_cats", "step_quarters", "acc_quarters","cadence30_quarters", "med_steps")
...
"model9" = c(strata("sex"), multivar_covs, "med_steps"),#adjusting for steps in cadence model
"model10" = c(strata("sex"), multivar_covs, "med_steps"), 
"model11" = exposure*c(strata("BMI_cats"))
...
cox_interaction <- coxph((Surv(age_entry_days, age_exit_days, RA_incident)) ~ 
...
lincom(cox_interaction, c("med_steps_std+med_steps_std:BMI_cats25.0-29.9",
...
cox_PRS <- coxph((Surv(age_entry_days, age_exit_days, RA_incident)) ~ 
                           med_steps_std*PRS_RA_RiskGroup,
                         data=dat)
...
cox_sex <- coxph((Surv(age_entry_days, age_exit_days, RA_incident)) ~ 
                           med_steps_std + med_steps_std:strata(sex),
                         data=dat)
...
```{r}
  model <- coxph(
    Surv(age_entry_days, age_exit_days, RA_incident) ~ med_steps_standardization*BMI_cats,
    data = dat
  )
  res <- summary(model)
  vcov(model)
