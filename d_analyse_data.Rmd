...
dat4yr <- fread("Data/prepped_steps_RA4years.csv", data.table = FALSE)
```
...
dat$cadence30_fifths <- qtile_cut(dat$CadencePeak30Adjusted.steps.min., probs = seq(0,1, by=0.2))
...
dat2yr$cadence30_fifths <- qtile_cut(dat2yr$CadencePeak30Adjusted.steps.min., probs = seq(0,1, by=0.2))
...
```{r}
dat4yr$step_fifths <- qtile_cut(dat4yr$med_steps, probs = seq(0, 1, by = 0.2))
dat4yr$acc_fifths <- qtile_cut(dat4yr$overall_activity, probs = seq(0, 1, by = 0.2), dp_label = 1)
...
```{r}
exposures <- c("step_cats", "step_fifths", "acc_fifths","cadence30_fifths")
...
"model4" = c("sex", multivar_covs,"BMI_cats"),
"model5" = c("sex", "PRS_RA_RiskGroup"),
"model6" = c("sex", multivar_covs,"PRS_RA_RiskGroup"), 
"model7" = c("sex", multivar_covs,"PRS_RA_RiskGroup","BMI_cats"), 
"model8" = c("sex", multivar_covs,"age_gp_crude") # this is just a crude check that adjustment for age using age as timescale has worked
)
...
No cadence plots in 4 year data 
4 year 
```{r}
for (exposure in exposures) {
  # SET UP RECORDING FRAME ======================================================
