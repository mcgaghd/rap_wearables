...
dir.create("plots")
output_dir <- "plots"
output_file <- file.path(output_dir, "baselinePsO.tiff")
...
## Write out
```{r}
write.csv(datPsO, "prepped_steps_PsAfinal.csv")
```
...
```{r}
primary <- "med_steps"
datPsO$med_steps_std <- standardise_column(datPsO[[primary]], 1000, TRUE)
hist(datPsO$med_steps_std)
```
...
```{r}
model_2 <- coxph(Surv(age_entry_days, age_exit_days_2, ProbDefPsA_incident) ~ 
                  med_steps_std + strata(ethnicity)+ as.factor(qualif) + as.factor(smoking) + 
                   as.factor(tdi_quarters) + as.factor(alcohol), data = datPsO)
