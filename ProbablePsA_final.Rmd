...
## Write out
```{r}
write.csv(dat, "prepped_steps_wholeCohort.csv")
dat <- fread("prepped_steps_wholeCohort.csv", data.table=FALSE)

# Check for duplicate column names
duplicated_names <- names(dat)[duplicated(names(dat))]
...
merged_data <- merged_data %>%
  mutate(
    arthralgia_6months = ifelse(event_dt < date_end_accel & event_dt >= (date_end_accel %m-% months(6)), TRUE, FALSE),
    arthralgia_1year = ifelse(event_dt < date_end_accel & event_dt >= (date_end_accel %m-% months(12)), TRUE, FALSE),
    arthralgia_2years = ifelse(event_dt < date_end_accel & event_dt >= (date_end_accel %m-% months(24)), TRUE, FALSE),
    arthralgia_5years = ifelse(event_dt < date_end_accel & event_dt >= (date_end_accel %m-% months(60)), TRUE, FALSE)
  )
dat_subset <- merged_data %>%
  group_by(eid) %>%
...
mediators <- c("BMI_cats", "SystemicTreatment")
...
```{r}
mediator_plot <- adjustment_plot(mediator_adjustments, save_plot = "outputs/mediator_analysis_inPsO.png")
```
