...
datStiffness_gp$event_dt <-  as.Date(datStiffness_gp$event_dt, format = "%Y-%m-%d")
...
## Write out
```{r}
write.csv(dat, "prepped_steps_PsAfinalAccelerometer.csv")
```
...
# Convert event dates to Date class
...
# Repeat for Enthesitis

# Repeat for Wrist and Hand Pain
...
```{r}
dat <- dat %>%
  mutate(total_2year = arthralgia_2year | myalgia_2year | fatigue_2year | swelling_2year |
                      BackPain_2year | KneePain_2year | HipPain_2year | FingerPain_2year | WristHandPain_2year)

dat <- dat %>%
  mutate(total_5year = arthralgia_5years | myalgia_5years | fatigue_5years | swelling_5years |
                      BackPain_5years | KneePain_5years | HipPain_5years | FingerPain_5years | WristHandPain_5years)
...
write.csv(dat, "prepped_steps_PsA_symptoms.csv", row.names = FALSE)
```

```{bash}
dx upload prepped_steps_PsA_symptoms.csv --dest users/mcgaghd/PsAfinal/prepped_steps_PsA_symptoms.csv
...
cox_BMI_cad <- coxph((Surv(age_entry_days, age_exit_days, PsATotal_incident)) ~ 
                           (SB) +as.factor(qualif) + as.factor(smoking) +as.factor(alcohol),
                          data=dat)
...
```{r}
datPsO$SB_std = (datPsO$SB - mean(datPsO$SB))/sd(datPsO$SB)
# Check distribution of variable
hist(datPsO$SB_std)
```

```{r}
datPsO$MVPA_std = (datPsO$MVPA - mean(datPsO$MVPA))/sd(datPsO$MVPA)
# Check distribution of variable
hist(datPsO$MVPA_std)
```

```{r}
sd(datPsO$med_steps) #3822.266
# Generate a standardization variable
datPsO$med_steps_std = (datPsO$med_steps - mean(datPsO$med_steps))/sd(datPsO$med_steps)
# Check distribution of variable
hist(datPsO$med_steps_std) 
```
...
datPsO$cad30_thirds <- qtile_cut(datPsO$CadencePeak30Adjusted.steps.min., probs = seq(0, 1, by = 1/3))
...
model_2 <- coxph(Surv(age_entry_days, age_exit_days_2, ProbDefPsA_incident) ~ 
                  as.factor(cad30_thirds) + as.factor(qualif) + as.factor(smoking) + 
