...
ModerateOpiates <- dat_scripts %>% 
  filter(
    grepl("Morphine|MST contin|Oramorph|Sevredol|Oxycodone|OxyContin|Buprenorphine|BuTrans|Pethidine|Tapentadol|Meptazinol|Meptazinol|Meptid|Transtec|DHC Continus", drug_name, ignore.case = TRUE) &
...
HighOpiates <- dat_scripts %>% 
  filter(
    grepl("Fentanyl|Durogesic|Hydromorphone|Palfium|Dextromoramide|Cyclimorph|Transtec", drug_name, ignore.case = TRUE) &
    !grepl("oint|ointment|eye|foam|Enema|ear|supp|cream|crm|oin|cre|lotion|topical|spray", drug_name, ignore.case = TRUE) 
  )
```
