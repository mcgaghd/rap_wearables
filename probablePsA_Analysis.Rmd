...
Full_withSymptoms <- coxph((Surv(age_entry_days, age_exit_days_2, ProbDefPsA_incident)) ~ 
                           (med_steps_std)+JointPainChronic +as.factor(sex) +as.factor(tdi_quarters)+as.factor(qualif) + as.factor(smoking) +as.factor(alcohol),
                          data=dat)
...
```{r}
Full_withSymptoms <- coxph((Surv(age_entry_days, age_exit_days_2, ProbDefPsA_incident)) ~ 
                           (SystemicTreatment) +as.factor(sex) +as.factor(tdi_quarters)+as.factor(qualif) + as.factor(smoking) +as.factor(alcohol),
                          data=dat)
...
dat$NoProdomalLabel <- !dat$JointPainChronic
...
dat <- dat %>%
  mutate(groups = case_when(
    ...
    JointPainChronic == TRUE ~ "Prodromal",
    TRUE ~ NA_character_
  ))
```
...
cox_prodrome <- coxph((Surv(age_entry_days, age_exit_days_2, ProbDefPsA_incident)) ~ 
                           med_steps_std+ as.factor(sex)+ as.factor(tdi_quarters) +as.factor(qualif)+
...
library(biostat3)
cox_prodrome <- coxph((Surv(age_entry_days, age_exit_days_2, ProbDefPsA_incident)) ~ 
                           med_steps_std*total_5year+ as.factor(sex)+ as.factor(tdi_quarters) +as.factor(qualif)+
                    + as.factor(smoking) + as.factor(alcohol),
                         data=dat)

cox_reduced <- coxph((Surv(age_entry_days, age_exit_days_2, ProbDefPsA_incident)) ~ 
                           med_steps_std+ as.factor(total_5year)+ as.factor(sex)+ as.factor(tdi_quarters) +as.factor(qualif)+
                    + as.factor(smoking) + as.factor(alcohol),
                         data=dat)
