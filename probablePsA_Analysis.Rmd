...
```{r}
dat_subset <- dat %>%
  inner_join(dat_i2 %>% select(eid, conditions_at_i2, date_inst_2), by = "eid")
```

Find incident self-report PsA 
```{r}
# 1 Self reported
dat_subset$self_reported_incidentPsA <- grepl("rheumatoid arthritis",
                                       dat_subset$conditions_at_i2)

sum(dat_subset$self_reported_incidentPsA)

sum(dat_subset$PsATotal_incident)
```
...
```{r}
# Add new columns for the Venn diagram
library(ggvenn)

dat$V1 <- NULL


datPsA_venn <- dat[dat$Prevalent_PsO_lessPsA == TRUE, ]

datPsA_venn$'Probable and Definite PsA Incident' <- datPsA_venn$ProbDefPsA_incident
datPsA_venn$'Definite PsA Incident (HES)' <- datPsA_venn$PsA_incident_HES

# Display the first few rows to verify the new columns
head(datPsA_venn)

plot <- ggvenn(datPsA_venn, c('Probable and Definite PsA Incident', 'Definite PsA Incident (HES)'),
