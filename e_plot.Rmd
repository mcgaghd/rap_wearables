...
# Create a histogram plot with spline terms
hist_plot <- ggplot(dat, aes(x = med_steps)) +
  geom_histogram(binwidth = 100, fill = "grey80") + # Adjust binwidth as needed
  geom_density(aes(y = ..count.. / sum(..count..)), fill = "blue", alpha = 0.5) + # Create a density plot with relative frequency
  labs(x = "Median Daily Steps", y = "Relative Frequency") + # Adjust labels as needed

# Create the spline plot
spline_plot <- ggplot(plot_dat, aes(x = steps, y = hr)) +
  geom_ribbon(aes(ymin = lowerCI, ymax = upperCI), fill = "grey80") +
  geom_line(color = "black") +
  scale_y_continuous(trans = "log", breaks = breaks) +
  labs(y = outcome_title, x = "Median Daily Steps") +
  geom_hline(yintercept = 1, linetype = "dashed") +
  geom_rug(data = dat[dat$med_steps < quantile(dat$med_steps, 0.99), ], mapping = aes(x = med_steps), inherit.aes = FALSE, linewidth = ruglinewidth, alpha = 0.5) +
  theme(panel.background = element_rect(fill = "white"))

# Combine the histogram and spline plot using cowplot
library(cowplot)
combined_plot <- plot_grid(hist_plot, spline_plot, ncol = 1, rel_heights = c(3, 7))
