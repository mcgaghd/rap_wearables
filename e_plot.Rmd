...
Need to take a subsample of dat in order to generate geom_rug plots akin to Small et al.
```{r}
percentile_99 <- quantile(dat$med_steps, 0.99)

# Filter values below the 99th percentile
filtered_data <- dat$med_steps[dat$med_steps <= 20000]

# Set the seed for reproducibility (optional)
set.seed(123)

# Take a random sample of 7,500 values from the filtered data
sample_size <- 7500
random_sample <- sample(filtered_data, size = sample_size, replace = FALSE)

# Create a new dataframe called datSubSample with the med_steps column
datSubSample <- data.frame(med_steps = random_sample)
```
...
shapeplot1 <- ggplot(datatoplot, aes(x = `mean_steps`, y = exp(estimate))) +
  ...
  theme(axis.line = element_line(size = 0.25, colour = "black", linetype=1),
 ...
 # Add rug plot
   geom_rug(data = datSubSample, aes(x = med_steps), sides = "b", linewidth = ruglinewidth,inherit.aes = FALSE, alpha=0.5)
  
  svg(paste0("outputs/shapeplot_stepfifths_", gsub(",", "_", adjustment), Sys.Date(), ".svg"), width = 10, height = 6)
  gridExtra::grid.arrange(grobs = list(shapeplot1), layout_matrix = cbind(1, 2), padding = unit(0.1, "line"))
  dev.off()
...
# Spline plots - USE THIS SPLINE PLOT 
...
for (outcome in outcomes){
  ...
  # MODEL ===============================================================================
  form <- as.formula(paste0("Surv(age_entry_days, age_exit_days, ", outcome, ") ~ pspline(med_steps, df = 3) + sex + ethnicity + tdi_quarters + qualif + smoking + alcohol + processed_meat + fresh_fruit + oily_fish + added_salt+PRS_RA_RiskGroup"))
  ...
  geom_hline(yintercept = 1, linetype = "dashed") + 
   ...
   #geom_rug(data = dat[dat$med_steps < quantile(dat$med_steps,0.99),],aes(x = med_steps), sides = "b", size = 0.001,inherit.aes = FALSE)
   # geom_rug(sides = "b", size = 0.5) +  # 'sides' controls the side of the axis for the rug 
 # xlab("med_steps") 
# geom_rug(data = datSubSample[datSubSample$med_steps < quantile(datSubSample$med_steps, 0.99), ], mapping = aes(x = med_steps), inherit.aes = FALSE, alpha = 0.5)
