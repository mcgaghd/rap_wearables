...
#step_fifths_results_tab <- fread("outputs/step_fifths_tab2yr.csv", data.table = FALSE)
#step_fifths_results_tab <- fread("outputs/step_fifths_tab4yr.csv", data.table = FALSE)
```
...
# CYCLE OVER ADJUSTMENTS=========================================
 for (adjustment in adjustments) {
  ...
  textsize <- 4
  ext <- c(0, 0)
  lim_rad_scale <- 1/min(fp_frame_step_fifths_RA_incident$stderr) + 1
...
ylims <- c(0, 2)
  ...
  ruglinewidth <- 0.03
  datatoplot <- fp_frame_step_fifths_RA_incident
...
shapeplot1 <- ggplot(datatoplot, aes(x = `mean_steps`, y = exp(estimate))) +
  ...
  scale_y_continuous(breaks = breaks, limits = ylims,expand = c(0, 0))+
 ...
 # Add rug plot
  geom_rug(data = dat[dat$med_steps < quantile(dat$med_steps, 0.99), ], mapping = aes(x = med_steps), inherit.aes = FALSE, linewidth = ruglinewidth, alpha = 0.2) +
  theme_bw()
  ...
  assign(paste0("shapeplot_RA_incident_step_fifths_", gsub(",", "_", adjustment)), shapeplot1)
   }
```
...
for (outcome in outcomes){
  ...
  # MODEL ===============================================================================
form <- as.formula(paste0("Surv(age_entry_days, age_exit_days, ", outcome, ") ~ pspline(med_steps, df = 3) + sex + ethnicity + tdi_quarters + qualif + smoking + alcohol + processed_meat + fresh_fruit + oily_fish + added_salt+BMI_cats"))
  ...
  geom_rug(data = dat[dat$med_steps < quantile(dat$med_steps, 0.99), ], 
        mapping = aes(x = med_steps), inherit.aes = FALSE,linewidth = ruglinewidth,alpha=0.2)+
    scale_y_continuous(breaks = breaks, limits = ylims,expand = c(0, 0)) +
    scale_x_continuous(limits = xlims,expand = c(0, 0)) +
...
Results: - select which sensitivity you want to run through 
```{r}
cad30_fifths_results_tab <- fread("outputs/cadence30_fifths_tab2yr.csv", data.table = FALSE) 
```

Organise results to plot and iteratively produce shape plots with different settings: 
```{r}
exposures <- c("cad30_fifths")
outcomes <- c("RA_incident")

adjustments <- unique(cad30_fifths_results_tab$Adjustment)
...
# CYCLE OVER ADJUSTMENTS=========================================
 for (adjustment in adjustments) {
  ...
  textsize <- 4
  ext <- c(0, 0)
  lim_rad_scale <- 1/min(fp_frame_cad30_fifths_RA_incident$stderr) + 1
  ...
  ruglinewidth <- 0.3
  datatoplot <- fp_frame_cad30_fifths_RA_incident
...
shapeplot1 <- ggplot(datatoplot, aes(x = `mean_cad30`, y = exp(estimate))) +
 ...
 # Add rug plot
  #geom_rug(data = dat[dat$CadencePeak30Adjusted.steps.min. > quantile(dat$CadencePeak30Adjusted.steps.min., 0.01) &
                    # dat$CadencePeak30Adjusted.steps.min. < quantile(dat$CadencePeak30Adjusted.steps.min., 0.99), ], #mapping = aes(x = CadencePeak30Adjusted.steps.min.), inherit.aes = FALSE, linewidth = ruglinewidth, alpha = 0.5)
  
# Plot like a CKB plot
  
  
  svg(paste0("outputs/shapeplot_cad30fifths_", gsub(",", "_", adjustment), Sys.Date(), ".svg"), width = 10, height = 6)
  
  dev.off()
  
  assign(paste0("shapeplot_RA_incident_cad30_fifths_", gsub(",", "_", adjustment)), shapeplot1)
   }
```
...
fifth1 <- quantile(dat$CadencePeak30Adjusted.steps.min., 0.2)
step_ref <-  mean(dat$CadencePeak30Adjusted.steps.min.[dat$CadencePeak30Adjusted.steps.min. < fifth1]) #5893.459 use mean steps in lowest category so match 
spl_ref <- predict(spl, step_ref) # spline terms at reference value of variable
spl_all <- predict(spl, steps) # spline terms across steps
L <- t(spl_all) - c(spl_ref) # matrix of spline terms, centred for reference value of variable
...
for (outcome in outcomes){
  ...
  # MODEL ===============================================================================
  form <- as.formula(paste0("Surv(age_entry_days, age_exit_days, ", outcome, ") ~ pspline(CadencePeak30Adjusted.steps.min., df = 3) + sex + ethnicity + tdi_quarters + qualif + smoking + alcohol + processed_meat + fresh_fruit + oily_fish + added_salt"))
  model <- coxph(form, data = dat)
