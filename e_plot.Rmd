...
dir.create("plots")
output_dir <- "plots"
output_file <- file.path(output_dir, "FullyAdjustedMRC2yr.tiff")
ggsave(output_file, shapeplot_RA_incident_step_quarters_1_ethnicity_tdi_quarters_qualif_smoking_alcohol_processed_meat_fresh_fruit_oily_fish, device = "tiff", width = 6, height = 8, dpi = 300)
```
...
for (outcome in outcomes){
  ...
  # MODEL ===============================================================================
form <- as.formula(paste0("Surv(age_entry_days, age_exit_days, ", outcome, ") ~ pspline(med_steps, df = 3) + strata(sex) + ethnicity+tdi_quarters+ qualif + smoking + alcohol + processed_meat + fresh_fruit + oily_fish +BMI_cats"))
...
```{r}
output_dir <- "plots"
output_file <- file.path(output_dir, "FullyAdjustedSplineMRC2yr.tiff")
