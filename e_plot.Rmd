...
shapeplot1 <- ggplot(datatoplot, aes(x = `mean_steps`, y = exp(estimate))) +
 ...
 # Add rug plot
  geom_rug(data = dat[dat$med_steps < quantile(dat$med_steps, 0.99), ], mapping = aes(x = med_steps), inherit.aes = FALSE, linewidth = ruglinewidth, alpha = 1)
...
# Second plot ---
  ...
  svg(paste0("outputs/shapeplot_stepfifths_", gsub(",", "_", adjustment), Sys.Date(), ".svg"), width = 13, height = 10)
...
#shapeplot2 <- ckbplotr::plot_like_ckb(plot = shapeplot2,
 #                       xlims          = xlims,
  ...
  assign(paste0("shapeplot_RA_incident_step_fifths_", gsub(",", "_", adjustment)), shapeplot1)
  assign(paste0("shapeplot_RA_incident_step_fifths_", gsub(",", "_", adjustment)), shapeplot2)
 }
...
filtered_data <- dat$med_steps[dat$med_steps <= percentile_99]

# Set the seed for reproducibility (optional)
set.seed(123)

# Take a random sample of 10,000 values from the filtered data
sample_size <- 2500
...
for (outcome in outcomes){
  ...
  geom_hline(yintercept = 1, linetype = "dashed") + 
    geom_rug(data = datSubSample, aes(x = med_steps), sides = "b", size = 0.08,inherit.aes = FALSE)
