...
```
```{r}
output_file <- file.path(output_dir, "FullyAdjustedCadenceplotFinal.tiff")
...
for (outcome in outcomes){
  ...
  # MODEL ===============================================================================
  form <- as.formula(paste0("Surv(age_entry_days, age_exit_days, ", outcome, ") ~ pspline(CadencePeak30Adjusted.steps.min., df = 3) + strata(sex) + ethnicity + qualif + tdi_quarters+ smoking + alcohol + processed_meat + fresh_fruit + oily_fish + added_salt"))
  model <- coxph(form, data = dat)
...
```{r}
output_file <- file.path(output_dir, "FullyAdjustedCadenceSplineFinal.tiff")
ggsave(output_file, plot_spline, device = "tiff", width = 6, height = 8, dpi = 300)
```
