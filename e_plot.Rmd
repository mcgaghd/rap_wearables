...
dir.create("plots")
output_dir <- "plots"
output_file <- file.path(output_dir, "FullyAdjusted_plot_4yr.tiff")
ggsave(output_file, shapeplot_RA_incident_step_quarters_1_ethnicity_tdi_quarters_qualif_season_wear_smoking_alcohol_processed_meat_fresh_fruit_oily_fish, device = "tiff", width = 6, height = 8, dpi = 300)
```
...
for (outcome in outcomes){
  ...
  # MODEL ===============================================================================
form <- as.formula(paste0("Surv(age_entry_days, age_exit_days, ", outcome, ") ~ pspline(med_steps, df = 3) + strata(sex) + ethnicity+tdi_quarters+ qualif + smoking + alcohol + processed_meat + fresh_fruit + oily_fish"))
...
```{r}
output_dir <- "plots"
output_file <- file.path(output_dir, "FullyAdjustedSpline_4yr.tiff")
...
```
```{r}
output_file <- file.path(output_dir, "FullyAdjustedCadenceplot_4yr.tiff")
ggsave(output_file, shapeplot_RA_incident_cad30_quarters_1_ethnicity_tdi_quarters_qualif_season_wear_smoking_alcohol_processed_meat_fresh_fruit_oily_fish, device = "tiff", width = 6, height = 8, dpi = 300)
```
...
for (outcome in outcomes){
  ...
  # MODEL ===============================================================================
  form <- as.formula(paste0("Surv(age_entry_days, age_exit_days, ", outcome, ") ~ pspline(CadencePeak30Adjusted.steps.min., df = 3) + strata(sex) + ethnicity + qualif + tdi_quarters+ smoking + alcohol + processed_meat + fresh_fruit + oily_fish"))
  model <- coxph(form, data = dat)
...
```{r}
output_file <- file.path(output_dir, "FullyAdjustedCadenceSplineFinal_4yr.tiff")
ggsave(output_file, plot_spline, device = "tiff", width = 6, height = 8, dpi = 300)
```
