...
dir.create("plots")
output_dir <- "plots"
output_file <- file.path(output_dir, "FullyAdjusted_plot.tiff")
ggsave(output_file, shapeplot_RA_incident_step_quarters_1_ethnicity_tdi_quarters_qualif_season_wear_smoking_alcohol_processed_meat_fresh_fruit_oily_fish_age_gp_crude, device = "tiff", width = 6, height = 8, dpi = 300)
```
...
for (outcome in outcomes){
  ...
  # MODEL ===============================================================================
form <- as.formula(paste0("Surv(age_entry_days, age_exit_days, ", outcome, ") ~ pspline(med_steps, df = 3) + strata(sex) + ethnicity+ qualif + smoking + alcohol + processed_meat + fresh_fruit + oily_fish +strata(BMI_cats)"))
...
```{r}
output_dir <- "plots"
output_file <- file.path(output_dir, "FullyAdjustedSpline.tiff")
...
```
```{r}
output_file <- file.path(output_dir, "FullyAdjustedCadence4yr.tiff")
ggsave(output_file, shapeplot_RA_incident_cad30_quarters_sex_ethnicity_qualif_season_wear_smoking_alcohol_processed_meat_fresh_fruit_oily_fish, device = "tiff", width = 6, height = 8, dpi = 300)
```
...
for (outcome in outcomes){
  ...
  # MODEL ===============================================================================
  form <- as.formula(paste0("Surv(age_entry_days, age_exit_days, ", outcome, ") ~ pspline(CadencePeak30Adjusted.steps.min., df = 3) + strata(sex) + ethnicity + qualif + smoking + alcohol + processed_meat + fresh_fruit + oily_fish + added_salt+ strata(BMI_cats)"))
  model <- coxph(form, data = dat)
...
```{r}
output_file <- file.path(output_dir, "FullyAdjustedCadenceSpline4yr.tiff")
ggsave(output_file, plot_spline, device = "tiff", width = 6, height = 8, dpi = 300)
```
