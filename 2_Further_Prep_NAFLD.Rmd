...
```{r}
ct1Model <- lm(CT1_liver ~ step_thirds+BMI_cats+corticosteroids, data = datMRIPsO)
summary(ct1Model)
```

```{r}
ct1Model <- lm(CT1_std ~ med_steps_std, data = datMRI_PsA)
summary(ct1Model)
```
...
model1 <- coxph((Surv(age_entry_days, age_exit_days, NAFLD_incident)) ~ 
                            med_steps_std+ as.factor(ethnicity)+as.factor(BMI_cats)+as.factor(smoking)+as.factor(methotrexate),
                         data=datPsO)
...
```{r}
datMRI$log_pdff <- log(datMRI$pdff)
```

```{r}
model <- lm(log_pdff ~ BMI_cats, data = datMRI)
summary(model)
```
