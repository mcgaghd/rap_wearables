...
```{r}
sed_1st_percentile <- quantile(dat$sedentary_overall_average, 0.01, na.rm = TRUE)  # Calculate the 1st percentile

# Filter the data to remove values less than the 1st percentile
dat <- dat %>%
  mutate(sed_clipped = pmax(sedentary_overall_average, sed_1st_percentile))
sd(dat$sed_clipped) #3822.266
# Generate a standardization variable
dat$sed_std = (dat$sed_clipped - mean(dat$sed_clipped))/sd(dat$sed_clipped)
# Check distribution of variable
hist(dat$sed_std)
```
