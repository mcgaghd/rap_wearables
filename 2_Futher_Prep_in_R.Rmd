...
```{r}
unique_eid_sumv2 <- datPsA_v2 %>%
      distinct(eid) %>%
     summarise(sum_unique_eid = sum(!is.na(eid)))
```
...
###GP data 
datPsO_v2$event_dt <- as.Date(datPsO_v2$event_dt, format = "%Y-%m-%d")
datPsO_v3$event_dt <- as.Date(datPsO_v3$event_dt, format = "%Y-%m-%d")
datPsA_v2$event_dt <- as.Date(datPsA_v2$event_dt, format = "%Y-%m-%d")
datPsA_v3$event_dt <- as.Date(datPsA_v3$event_dt, format = "%Y-%m-%d")
...
```{r}
###GP data first - prevalent cases 

PsO_read2codes <- ("^M16..$|^M16z.$|^M161z$|^M1610$|^M1616$|^M161F$|^M161.$|^14F2.$|^M166.$|^M16y0$|^M161A$|^M161D$|^M161B$|^M161H$|^Myu30$|^M1614$|^M16y.$|^M161G$|^M1619$|^M161J$")

PsO_read3codes <- ("^M161z$|^XE1Cq$|^M16..$|^M1616$|^M1610$|^X506Y$|^X506Q$|^X506i$|^X506K$|^XaNXt$|^M16z.$|^X506C$|^X506T$|^X506I$|^X506m$|^M161D$|^M161B$|^Myu30$|^M161.$|^M1614$|^XaYOx$|^X506B$|^X506e$|^XaNXa$|^M16y.$|^X506Z$|^M161A$|^X506k$|^X506g$|^X506d$|^XaNxT$|^XE1CQ$|^X506b$|^M161F$|^X506L$|^X506f$|^M1612$|^X506N$|^X506O$|^M1611$|^M1613$|^X506U$|^X506a$|^X506c$|^M161E$|^X506F$|^X506P$|^M1617$|^X506S$|^X506l$|^M161G$|^M1618$|^X506h$|^M161C$|^XaYOX$|^X506E$|^M1619$")

PsA_read2codes <- ("^M160.$|^M160z$|^M1601$|^M1602$|^N0452$|^M1600$|^Nyu13$")
PsA_read3codes <- ("^M160.$|^M160z$|^X701W$|^Nyu13$|^X701v$|^X7028$|^M1600$|^X701U$|^X7026$")


dat <- merge(
  dat,
  GP_disease_lookup(datPsO_v3, dat, PsO_read2codes, PsO_read3codes,
                     "prevalent", "gp_prevalent_PsOv3"),
  by = "eid",
  all.x = TRUE,
  suffixes = c("", "dup")
)
...
dat <- merge(
  dat,
  GP_disease_lookup(datPsO_v2, dat, PsO_read2codes, PsO_read3codes,
                     "prevalent", "gp_prevalent_PsOv2"),
  by = "eid",
  all.x = TRUE,
  suffixes = c("", "dup")
)
...
dat <- merge(
  dat,
  GP_disease_lookup(datPsA_v3, dat, PsA_read2codes, PsA_read3codes,
                     "prevalent", "gp_prevalent_PsA_v3"),
  by = "eid",
  all.x = TRUE,
  suffixes = c("", "dup")
)
...
dat <- merge(
  dat,
  GP_disease_lookup(datPsA_v2, dat, PsA_read2codes, PsA_read3codes,
                     "prevalent", "gp_prevalent_PsA_v2"),
  by = "eid",
  all.x = TRUE,
  suffixes = c("", "dup")
)
...
dat$total_prevalent_PsO <- dat$self_reported_PsO | dat$hes_prevalent_PsO | dat$gp_prevalent_PsOv2 | dat$gp_prevalent_PsOv3

dat$total_prevalent_PsA <- dat$self_reported_PsA | dat$hes_prevalent_PsA | dat$gp_prevalent_PsA_v2 |dat$gp_prevalent_PsA_v3
...
dat <- dat %>%
  mutate(PsA_incident_gp = ifelse(hes_prevalent_PsA | gp_prevalent_PsA | self_reported_PsA, FALSE, gp_incident_PsA))

dat <- merge(
  dat,
  GP_disease_lookup(dat_gp, dat, PsA_read2codes, PsA_read3codes,
                     "date", "PsA_incident_date_gp"),
  by = "eid",
  all.x = TRUE,
  suffixes = c("", "dup")
)

dat$PsA_incident_date_gp[!dat$PsA_incident_gp] <- NA
dat$PsA_incident_date_gp <- as.Date(dat$PsA_incident_date_gp, format = "%Y-%m-%d")

#We only gain 6 cases by adding those with gp Incident PsA and no HES incident cases so given issues with follow up time and censoring - we will only use GP data to collect on prevalent disease 
```

```{r}
#Incident total
dat$PsA_incident_total <- dat$PsA_incident | dat$PsA_incident_gp 
```
