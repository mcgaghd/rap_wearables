...
dat_sleep <- fread("cancer_sleep.csv", data.table = FALSE)
dat_sleepQA <- fread("cancer_qa_nov_09_2023.csv", data.table = FALSE)
...
datEMS_gp$event_dt <-  as.Date(datEMS_gp$event_dt, format = "%Y-%m-%d")
...
datEMS_gpFirst <- datEMS_gp %>%
  rename(event_dt_EMS = event_dt) %>%
  group_by(eid) %>%
  arrange(event_dt_EMS) %>%
  slice(1) %>%
  ungroup()

datEMS_gpFirst$event_dt_EMS <- as.Date(datEMS_gpFirst$event_dt_EMS , format = "%Y-%m-%d")

dat <- dat %>%
  left_join(datEMS_gpFirst, by = c("eid" = "eid"))

dat <- dat %>%
  mutate(gp_EMS = if_else(is.na(gp_EMS), FALSE, gp_EMS))
```
...
dat <- dat %>%
  mutate(
    ...
    prePsA_psoriasis2year = if_else(
      total_prevalent_PsO & 
      abs(difftime(date_end_accel, PsA_incident_total_date, units = "days")) <= 730, # 730 days approximates 2 years
      TRUE,
      FALSE,
...
dat <- dat %>%
  mutate(prePsA_late = total_prevalent_PsO & PsA_incident_total & !prePsA_psoriasis2year)
```
...
```{r}
# The lists of ICD codes we will consider
CVD_icd10_codes <- "^I20|I21[0-9]|I22[0-9]|I23[0-8]|I50\\.?[0-9]?|I472|I49[0]|I46[0-9]|I470|I63.|I64.|G45.|4109|428[019]$"
CVD_icd9_codes <- "4109|4280|4281|4289"



# 1 Self reported
dat$self_reported_CVD <- grepl("angina|heart attack/myocardial infarction|subarachnoid haemorrhage|stroke|ischaemic stroke|transient ischaemic attack 
                               (tia)|subdural haemorrhage/haematoma|cerebral aneurysm", dat$conditions_at_baseline_2006)


# Then we look-up those with rheumatoid arthritis
dat <- merge(
  dat,
  hes_disease_lookup(dat_hes, dat, CVD_icd10_codes, CVD_icd9_codes,
                     "prevalent", "hes_prevalent_CVD"),
  by = "eid",
  all.x = TRUE,
  suffixes = c("", "dup")
)
...
##create new total RA 
dat$total_prevalent_CVD <- dat$self_reported_CVD | dat$hes_prevalent_CVD
...
dat <- merge(
  dat,
  death_disease_lookup(dat_death_cause, dat, CVD_icd10_codes,
                       "death_CVD"),
  by = "eid",
  all.x = TRUE,
  suffixes = c("", "dup")
...
dat$incident_CVD <- dat$hes_incident_CVD | dat$death_CVD

dat$CVD_incident <- dat$incident_CVD & (!dat$total_prevalent_CVD)




dat$CVD_incident_date[!dat$CVD_incident] <- NA
dat$CVD_incident_date <- as.Date(dat$CVD_incident_date, format = "%Y-%m-%d")
```
...
dat$date_fu[dat$CVD_incident] <- dat$CVD_incident_date[dat$CVD_incident]
```
...
Sleep 
```{r}
datSleep <- full_join(dat_sleep, dat_sleepQA, by = "pid")
datSleep <- datSleep %>% rename(eid = pid)
dat <- inner_join(dat, datSleep, by = "eid")
```
...
ok_columns <- c("is_parsing_ok", "acc_avg_ok", "ok_calibration", "good_day_num", "restless_ok", "daylight_saving_ok")

# Initialize the number of rows before filtering
nb_initial <- nrow(dat)

for (col in ok_columns) {
    # Count the number of rows before this iteration of filtering
    nb_before_filtering <- nrow(dat)

    # Filter the dat data frame to keep only rows where the current column is TRUE
    dat <- dat[dat[[col]], ]

    # Count the number of rows after filtering
    nb_after_filtering <- nrow(dat)

    # Update the exclusion table
    tab_exc <- rbind(
        tab_exc,
        data.frame(
            "Exclusion" = paste("Failed", col),
            "Number_excluded" = nb_before_filtering - nb_after_filtering,
            "Number_remaining" = nb_after_filtering
        )
    )
}

# Finally, update the exclusion table for overall exclusion
tab_exc <- rbind(
    tab_exc,
    data.frame(
        "Exclusion" = "Total Exclusion after all checks",
        "Number_excluded" = nb_initial - nrow(dat),
...
datPsA_venn <- dat[dat$total_prevalent_PsA == TRUE, ]


plot <- ggvenn(datPsA_venn, c("self_reported_PsA", "hes_prevalent_PsA", "PrevalentPsA_gp"))

# Adjust the theme to make the text smaller
plot + theme(text = element_text(size = 4))
...
plot <- ggvenn(datPsO_venn, c("self_reported_PsO", "hes_prevalent_PsO", "PrevalentPsO_gp"))
...
```{r}
datLiver <- dat %>%
  inner_join(dat_PDFF %>% select(eid, pdff, date_pdff), by = "eid")
```
...
```{r}
datLiver <- datLiver %>%
  filter(!is.na(date_fu))
```

```{r}
datLiver$date_fu[datLiver$NAFLD_mri] <- datLiver$date_pdff[datLiver$NAFLD_mri]
```

We calculate follow up time (i.e. total time on study):
```{r}
datLiver$fu_time <-as.double(difftime(datLiver$date_fu, datLiver$date_end_accel, units = "days"))
```

We want to analyse the data using age as the timescale, so we add a variable for age at exit in days:
```{r}
datLiver$age_exit_days <- as.double(datLiver$age_entry_days + datLiver$fu_time)
datLiver$age_exit_days2 <-  as.double(difftime(datLiver$date_fu, datLiver$approx_dob, units = "days")) # calculation in an alternative way just so we can implement a logic check

# Logic check 
if (!isTRUE(all.equal(datLiver$age_exit_days, datLiver$age_exit_days2))){
    stop("Different methods of calculating age at exit give different answers")
}
```
...
```{r}
datLiver$step_thirds <- qtile_cut(datLiver$med_steps, probs = seq(0, 1, by = 1/3))
```
