...
Read in the GP data tables for each of PsO and PsA and for Read V2 and V3
```{r}
...
```{r}
datPsAtotal<- dplyr::bind_rows(datPsA_v2, datPsA_v3)
datPsOtotal<- dplyr::bind_rows(datPsO_v2, datPsA_v3)
dat_gp <- dplyr::bind_rows(datPsOtotal, datPsAtotal)

#Remove NAs where blank for date as unable to ascertain prevalent or incident 
dat_gp <- dat_gp %>%
...
PsO_read2codes <- "^(^M16[^0|^2|^3|^4|^5].*$|Myu30|14F2.)"
PsO_read3codes <- "^M16[^0|^2|^3|^4|^5].*$|Myu30|X506Y|X506Z|X506a|X506b|X506c|X506d|X506e|X506f|X506g|X506h|X506i|X506j|X506k|X506l|X506m|X506n|X506o|X506p|X506q|X506r|X506s|X506t|X506u|X506v|XE1CQ|XaNXa|XaNXt|XaYOx"
PsA_read2codes <- "^(?i)(M160|M1602|N0452|Nyu13)"
PsA_read3codes <- "^(?i)(M160(.*)|M1600|M160z|Nyu13|X701u|X701v|X701w|X7026|X7027|X7028)"

#Psoriasis first 
dat <- merge(
  dat,
  GP_disease_lookup(dat_gp, dat, PsO_read2codes, PsO_read3codes,
                     "prevalent", "gp_prevalent_PsO"),
  by = "eid",
  all.x = TRUE,
  suffixes = c("", "dup")
)
...
dat <- merge(
  dat,
  GP_disease_lookup(dat_gp, dat, PsA_read2codes, PsA_read3codes,
                     "prevalent", "gp_prevalent_PsA"),
  by = "eid",
  all.x = TRUE,
  suffixes = c("", "dup")
)
...
dat$total_prevalent_PsO <- dat$self_reported_PsO | dat$hes_prevalent_PsO | dat$gp_prevalent_PsO

dat$total_prevalent_PsA <- dat$self_reported_PsA | dat$hes_prevalent_PsA | dat$gp_prevalent_PsA

##Create new variable which is psoriasis ONLY
dat$Prevalent_PsO_lessPsA <- dat$total_prevalent_PsO & !dat$total_prevalent_PsA

#Incident total
dat$PsA_incident_total <- dat$PsA_incident | dat$gp_incident_PsA 
```
...
```{r}
tab_exc2 <- data.frame("Exclusion" = "Starting cohort", "Number_excluded" = NA, "Number_remaining" = nrow(dat))
```
