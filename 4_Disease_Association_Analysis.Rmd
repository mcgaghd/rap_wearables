...
Model for timing 
```{r}
model_stepsTiming<- lm(StepsRatioMedDiff ~ AnalysisLabel + as.factor(sex)  + as.factor(age_gp)+as.factor(season_wear),  data = datTime)
...
label(dat$major_coronary_disease_charlson_CH) <- "Coronary artery disease"
...
```
```{r}
emmip(catcat,   cci_huang~AnalysisLabel  ,CIs=TRUE)
```
