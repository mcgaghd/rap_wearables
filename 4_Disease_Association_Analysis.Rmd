...
```{r}
pkgs <- c("data.table", "ggplot2", "survival", "table1", "IRdisplay", "dplyr", "emmeans", "data.table","gridExtra","lubridate","stargazer","table1") # packages we need
...
Duplicate for datTime which will be removed 
Tidying variables for Scott steps analysis 
```{r} 
#Create a singular factor column which reads 0 - healthy controls i.e. non psoriatic disease, 1 psoriasis and 2 = PsA 

#First create non-psoriatic group 
datTime$NonPsoriaticDisease <- !datTime$Prevalent_PsO_lessPsA & !datTime$total_prevalent_PsA

#Now we want to create a new column called FactorAnalysis which the above 3 factors 
datTime$AnalysisCategory <- ifelse(datTime$NonPsoriaticDisease, 0,
                              ifelse(datTime$Prevalent_PsO_lessPsA, 1,
                                     ifelse(datTime$total_prevalent_PsA, 2, NA)))

#Set up a corresponding label to these categories 
labels2 <- c("NonPsoriaticDisease", "PsoriasisOnly", "PsA")

datTime$AnalysisLabel <- factor(datTime$AnalysisCategory, levels = 0:2, labels = labels2)
```

Round time to nearest 15 minutes for adjusted Q1, Q2 and Q3 
```{r}
datTime$StepsQ1DayAvgAdjustedAt<- as.POSIXct(datTime$StepsQ1DayAvgAdjustedAt, format = "%H:%M:%S")

# Format the time column to HH:MM:SS
datTime$StepsQ1DayAvgAdjustedAt <- format(datTime$StepsQ1DayAvgAdjustedAt, format = "%H:%M:%S")
```

Need a minutes since midnight for Q1 in order to plug into lm(). We also need to remove anyone with abnormally early times as these likely reflect shift workers. 
```{r}
minutes_since_midnight <- function(time_str) {
...
# Apply the function to the dataframe Q1 
datTime$StepsQ1DayAvgAdjustedAtMinutes <- sapply(datTime$StepsQ1DayAvgAdjustedAt, minutes_since_midnight)
#Q2
datTime$StepsQ2DayAvgAdjustedAtMinutes <- sapply(datTime$StepsQ2DayAvgAdjustedAt, minutes_since_midnight)
#Q3 
datTime$StepsQ3DayAvgAdjustedAtMinutes <- sapply(datTime$StepsQ3DayAvgAdjustedAt, minutes_since_midnight)

# Apply the function to the dataframe Q1 
datTime$StepsQ1DayMedAdjustedAtMinutes <- sapply(datTime$StepsQ1DayMedAdjustedAt, minutes_since_midnight)
#Q2
datTime$StepsQ2DayMedAdjustedAtMinutes <- sapply(datTime$StepsQ2DayMedAdjustedAt, minutes_since_midnight)
#Q3 
datTime$StepsQ3DayMedAdjustedAtMinutes <- sapply(datTime$StepsQ3DayMedAdjustedAt, minutes_since_midnight)

datTime <- datTime[datTime$StepsQ1DayAvgAdjustedAtMinutes >= 240, ]
datTime <- datTime[datTime$StepsQ1DayMedAdjustedAtMinutes >= 240, ]

```

Model for timing 
```{r}
model_stepsTiming<- lm(StepsRatio ~ AnalysisLabel ,  data = datTime)
summary(model_stepsTiming)
  confint(model_stepsTiming)
  
emmeans_model_stepsTiming_np<- emmeans(model_stepsTiming, specs = "AnalysisLabel")
emmeans_model_stepsTiming <- emmeans(model_stepsTiming, specs = pairwise~ AnalysisLabel)
summary(emmeans_model_stepsTiming_np)
summary(emmeans_model_stepsTiming)
confint(emmeans_model_stepsTiming)

```

Compute new variable which is Q2 - Q1 / Q3 - Q2 

```{r}
datTime <- datTime %>%
  mutate(StepsRatio = (StepsQ2DayAvgAdjustedAtMinutes - StepsQ1DayAvgAdjustedAtMinutes) /
                      (StepsQ3DayAvgAdjustedAtMinutes - StepsQ2DayAvgAdjustedAtMinutes))

```

```{r}
datTime <- datTime %>%
  mutate(StepsRatioMed = (StepsQ2DayMedAdjustedAtMinutes - StepsQ1DayMedAdjustedAtMinutes) /
                      (StepsQ3DayMedAdjustedAtMinutes - StepsQ2DayMedAdjustedAtMinutes))

```

```{r}
datTime <- datTime %>%
  mutate(StepsRatioMedDiff = (StepsQ3DayMedAdjustedAtMinutes - StepsQ1DayMedAdjustedAtMinutes))

```


Create a plot which is just raw acceleration by hour across each of the 3 groups 
```{r}
...
# Make table
tab_desc <- table1::table1(~ age_entry_years + sex + ethnicity + tdi_cats  + smoking + alcohol + BMI +BMI_cats | groups, 
...
label(dat$cci_huang) <- "CCI by categories"
dat$cci_huang <- factor(dat$cci_huang, levels = c("None", "Mild to Moderate", "Severe"))


tab_descComorbid <- table1::table1(~ cci +cci_huang+ major_coronary_disease_charlson_CH +cerebrovascular_disease_charlson_CH+ congestive_heart_failure_charlson_CH + diabetes_charlson_CH +liver_disease_charlson_CH+cancer_charlson_CH+copd_charlson_CH+depression  | groups, 
                           data = dat 
                          #  ,render.cont = my_render_cont
)
tab_descComorbid
...
step_plotUnA <- ggplot()+
  ...
  scale_y_continuous(breaks = seq(0,11000, by = 400), name = "Unadjusted Mean Daily Step Count")+
  ...
  theme(axis.text.x.bottom = element_text(size = 10)) + 
   labs(caption = "Minimally adjusted model for age (5-year bands) and sex") +
...
step_plot1 <- ggplot(steps, aes(x = Quality, y = Mean, ymin = LowerCI, ymax = UpperCI, color = factor(Quality))) +
  ...
  theme(axis.text.y.left = element_text(size = 10)) +
  theme(axis.text.x.bottom = element_text(size = 10)) +
  labs(caption = "Partially adjusted model for age (5-year bands), sex, Townsend (by quintiles), season of wear, alcohol and smoking") +
  theme(plot.caption = element_text(hjust = 0))
...
model_stepsCCI <- lm(med_steps ~ AnalysisLabel + as.factor(sex)  + as.factor(age_gp) + as.factor(season_wear) +
                        as.factor(alcohol) + as.factor(tdi_cats) + as.factor(smoking)+cci,  data = dat)
...
step_plotCCI <- ggplot()+
  ...
  coord_cartesian(ylim = c(8000,10000))+
  xlab("")+
  geom_hline(yintercept=stepsCCI$Mean[1], linetype="dashed", color = "black", size=0.3)+
  theme(axis.title.x = element_text(vjust=-1))+
  theme(axis.text.y.left = element_text(size = 10))+
  theme(axis.text.x.bottom = element_text(size = 10)) + 
   labs(caption = "Model adjusted for age (5-year bands), sex, Townsend (by quintiles), season of wear, alcohol, smoking and CCI") +
  theme(plot.caption = element_text(hjust=0))
...
cad_plotMin <-ggplot()+
  ...
  xlab("")+
  geom_hline(yintercept=cadenceMin$Mean[1], linetype="dashed", color = "black", size=0.3)+
  theme(axis.title.x = element_text(vjust=-1))+
  theme(axis.text.y.left = element_text(size = 10))+
  theme(axis.text.x.bottom = element_text(size = 10))+ 
   labs(caption = "Minimally adjusted model for age (5-year bands) and sex") +
...
CCI model 
```{r}
model_cadenceCCI<- lm(CadencePeak30Adjusted.steps.min. ~ AnalysisLabel + as.factor(age_gp)+as.factor(sex)+as.factor(season_wear)+ as.factor(tdi_cats) + as.factor(alcohol) + as.factor(smoking) +cci,  data = dat)
...
cad_plotCCI <-ggplot()+
  ...
  coord_cartesian(ylim = c(70,100))+
  xlab("")+
  geom_hline(yintercept=cadenceCCI$Mean[1], linetype="dashed", color = "black", size=0.3)+
  theme(axis.title.x = element_text(vjust=-1))+
  theme(axis.text.y.left = element_text(size = 10))+
  theme(axis.text.x.bottom = element_text(size = 10))+ 
   labs(caption = "Minimally adjusted model for age (5-year bands) and sex") +
  theme(plot.caption = element_text(hjust=0))+
  labs(caption = "Partially adjusted model for age (5-year bands), sex, Townsend (by quintiles), season of wear, alcohol and smoking and CCI") +
  theme(plot.caption = element_text(hjust=0))
cad_plotCCI 
```
...
model_acc1 <- lm(overall_activity ~ AnalysisLabel + as.factor(sex) + BMI + age_entry_years + as.factor(season_wear)  +
...
```{r}
model_sedentary1 <- lm(sedentary_overall_average*1440~ AnalysisLabel + as.factor(sex) + BMI + age_entry_years + as.factor(season_wear)  +
                      + as.factor(ethnicity) + as.factor(alcohol) + tdi_raw + as.factor(smoking),  data = dat)
...
```
```{r}
model_MVPA1 <- lm(MVPA_overall_average*1440~ AnalysisLabel + as.factor(sex) + BMI + age_entry_years + as.factor(season_wear)  +
                      + as.factor(ethnicity) + as.factor(alcohol) + tdi_raw + as.factor(smoking),  data = dat)
summary(model_MVPA1)
confint(model_MVPA1)
...
mvpa_plot <-ggplot()+
  ...
  xlab("")+
  geom_hline(yintercept=MVPA$Mean[1], linetype="dashed", color = "black", size=0.3)+
  theme(axis.title.x = element_text(vjust=-1))+
  theme(axis.text.y.left = element_text(size = 10))+
  theme(axis.text.x.bottom = element_text(size = 10))
mvpa_plot
...
sed_plot <-ggplot()+
  ...
  theme(panel.grid.minor =  element_blank())+
  theme(legend.position="none")+
  scale_y_continuous(breaks = seq(500,600, by = 6), name = "Adjusted Mean Sedentary time (Minutes/Day)")+
  coord_cartesian(ylim = c(540,600))+
  xlab("")+
  geom_hline(yintercept=sed$Mean[1], linetype="dashed", color = "black", size=0.3)+
  theme(axis.title.x = element_text(vjust=-1))+
  theme(axis.text.y.left = element_text(size = 10))+
  theme(axis.text.x.bottom = element_text(size = 10))
sed_plot
