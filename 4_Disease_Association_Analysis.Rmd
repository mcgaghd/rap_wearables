...
```{r}
pkgs <- c("data.table", "ggplot2", "survival", "table1", "IRdisplay", "dplyr", "emmeans", "data.table","gridExtra","lubridate", "table1","stargazer") # packages we need
...
```{r}
comorbid <- fread("Data/charlson_data_10yr.csv", data.table = FALSE)
```

```{r}
dat <- merge(dat, comorbid, by = "eid", all.x = TRUE)
```

```{r}
#Rename cerebrovascular disease 
dat <- dat %>%
  rename(cerebrovascular_disease_charlson.y = "cerebrovascular disease_charlson.y")
```

Comorbidities table 
```{r}
label(dat$cci) <- "Charlson Comorbidity Index"
label(dat$major_coronary_disease_charlson.y) <- "Coronary artery disease"
label(dat$congestive_heart_failure_charlson.y) <- "Congestive heart failure"
label(dat$diabetes_charlson.y) <- "Diabetes"
label(dat$liver_disease_charlson.y) <- "Chronic liver disease"
label(dat$cancer_charlson.y) <- "Any cancer"
label(dat$depression) <- "Depression"
label(dat$cerebrovascular_disease_charlson.y) <- "Cerebrovascular disease"
label(dat$copd_charlson.y) <- "COPD"
label(dat$cci_huang) <- "CCI by categories"
dat$cci_huang <- factor(dat$cci_huang, levels = c("None", "Mild to Moderate", "Severe"))


tab_descComorbid <- table1::table1(~ cci + +cci_huang+ major_coronary_disease_charlson.y +cerebrovascular_disease_charlson.y+ congestive_heart_failure_charlson.y + diabetes_charlson.y +liver_disease_charlson.y+cancer_charlson.y+copd_charlson.y+depression  | groups, 
                           data = dat 
                          #  ,render.cont = my_render_cont
)
tab_descComorbid
