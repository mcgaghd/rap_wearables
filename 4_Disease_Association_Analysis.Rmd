...
# Make table
tab_desc <- table1::table1(~ age_entry_years + sex + ethnicity + tdi_cats + age_education_revalued_raw + smoking + alcohol + BMI | overall_activity_quarters, 
                           data = dat, 
...
model_steps1 <- lm(med_steps ~ AnalysisLabel + as.factor(sex)  + age_entry_years + as.factor(season_wear) +
                     + as.factor(ethnicity) + as.factor(alcohol) + `Townsend deprivation index at recruitment` + BMI+ as.factor(smoking),  data = dat)
...
model_acc1 <- lm(overall_activity ~ AnalysisLabel + as.factor(sex) + BMI + age_entry_years + as.factor(season_wear)  +
                  as.factor(ethnicity) + as.factor(alcohol) + `Townsend deprivation index at recruitment`+ as.factor(smoking),  data = dat)
summary(model_acc1)
confint(model_acc1)
