...
dat <- dat %>%
  mutate(AnalysisCategory2 = case_when(
...
dat$AnalysisLabel2 <- factor(dat$AnalysisCategory2, levels = 0:4, labels = labels3)
```

```{r}
model_steps1 <- lm(med_steps ~ AnalysisLabel2 + as.factor(sex)  + as.factor(age_gp) + as.factor(season_wear)+ as.factor(tdi_cats) + as.factor(alcohol) + as.factor(smoking),  data = dat)
summary(model_steps1)
  confint(model_steps1)
  
emmeans_model_steps1_np<- emmeans(model_steps1, specs = "AnalysisLabel2")
emmeans_model_steps1 <- emmeans(model_steps1, specs = pairwise~ AnalysisLabel2)
...
steps <- summary(emmeans_model_steps1_np, specs = "AnalysisLabel2")[1:5, c(1, 2, 6, 5)]
