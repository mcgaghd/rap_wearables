...
```{r}
dat_PsA <- dat[dat$total_prevalent_PsA == TRUE, ]
...
dat <- dat %>%
  mutate(PsoriaticComorbid = case_when(
    ...
    PsOComorbid ~ 2,
    PsOwithoutComorbid ~ 1,
    PsAComorbid ~ 4,
    PsAwithoutComorbid ~ 3,
    TRUE ~ NA_real_  # default case if none of the above conditions are met
  ))
                                     
#Set up a corresponding label to these categories 
labels3 <- c("NonPsoriaticDisease", "PsO + no comorbidity", "PsO + comorbidity", "PsA + no comorbidity", "PsA + comorbidity")

dat$PsoriaticComorbid <- factor(dat$PsoriaticComorbid, levels = 0:4, labels = labels3)
```
...
steps <- summary(emmeans_model_steps1_np, specs = "PsoriaticComorbid")[1:5, c(1, 2, 6, 5)]
names(steps) <- c("Quality", "Mean", "UpperCI", "LowerCI")
steps$Quality <- c("Non-Psoriatic", "PsO", "PsO + comorbidity", "PsA", "PsA + comorbidity")
steps$Quality <- factor(steps$Quality, levels = c("Non-Psoriatic", "PsO", "PsO + comorbidity", "PsA", "PsA + comorbidity"))
...
step_plot1 <- ggplot(steps, aes(x = Quality, y = Mean, ymin = LowerCI, ymax = UpperCI, color = factor(Quality))) +
  ...
  labs(caption = "Model adjusted for age (10-year bands), sex, Townsend (by quintiles), season of wear, alcohol and smoking", size=10) +
...
```{r}
model_cadenceCCI<- lm(CadencePeak30Adjusted.steps.min. ~ AnalysisLabel2 + as.factor(age_gp)+as.factor(sex)+as.factor(season_wear)+ as.factor(tdi_cats) + as.factor(alcohol) + as.factor(smoking),  data = dat)
summary(model_cadenceCCI)
  confint(model_cadenceCCI)
  
emmeans_model_cadenceCCI_np<- emmeans(model_cadenceCCI, specs = "AnalysisLabel2")
emmeans_model_cadenceCCI<- emmeans(model_cadenceCCI, specs = pairwise~ AnalysisLabel2)
...
Plot of CCI model 
```{r}
cadenceCCI <- summary(emmeans_model_cadenceCCI_np, specs = "AnalysisLabel2")[1:5,c(1,2,6,5)]
names(cadenceCCI) <- c("Quality", "Mean", "UpperCI", "LowerCI")
cadenceCCI$Quality <- c("Non-Psoriatic", "PsO", "PsO + comorbidity", "PsA", "PsA + comorbidity")
cadenceCCI$Quality <- factor(cadenceCCI$Quality, c("Non-Psoriatic", "PsO", "PsO + comorbidity", "PsA", "PsA + comorbidity"))
...
dat <- dat %>%
  mutate(PsoriaticDepression = case_when(
    NonPsoriaticDisease ~ 0,
    PsODepression ~ 2,
    PsOwithoutDepression ~ 1,
    PsADepression ~ 4,
    PsAwithoutDepression ~ 3,
    TRUE ~ NA_real_  # default case if none of the above conditions are met
  ))
                                     
#Set up a corresponding label to these categories 
labels3 <- c("NonPsoriaticDisease", "PsO + no depression", "PsO + depression", "PsA + no depression", "PsA + depression")

dat$PsoriaticDepression <- factor(dat$PsoriaticDepression, levels = 0:4, labels = labels3)
```
...
# Define the data frame for plotting
steps <- summary(emmeans_model_steps1_np, specs = "PsoriaticDepression")[1:5, c(1, 2, 6, 5)]
names(steps) <- c("Quality", "Mean", "UpperCI", "LowerCI")
steps$Quality <- c("Non-Psoriatic", "PsO", "PsO + depression", "PsA", "PsA + depression")
steps$Quality <- factor(steps$Quality, levels = c("Non-Psoriatic", "PsO", "PsO + depression", "PsA", "PsA + depression"))
...
Plot of CCI model 
```{r}
cadenceCCI <- summary(emmeans_model_cadenceCCI_np, specs = "PsoriaticDepression")[1:5,c(1,2,6,5)]
names(cadenceCCI) <- c("Quality", "Mean", "UpperCI", "LowerCI")
cadenceCCI$Quality <- c("Non-Psoriatic", "PsO", "PsO + depression", "PsA", "PsA + depression")
cadenceCCI$Quality <- factor(cadenceCCI$Quality, c("Non-Psoriatic", "PsO", "PsO + depression", "PsA", "PsA + depression"))
...
```{r}
model_SE_UnA <- lm(overall_mean_tst_min ~ total_prevalent_PsA + as.factor(sex)  + as.factor(age_gp),  data = dat)
...
tiff("step_plot1.tiff", width = 8, height = 6, units = "in", res = 300)
