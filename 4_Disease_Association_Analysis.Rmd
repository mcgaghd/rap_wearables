...
```{r}
pkgs <- c("data.table", "ggplot2", "survival", "table1", "IRdisplay", "dplyr", "emmeans", "data.table","gridExtra","lubridate", "table1") # packages we need
...
# Make table
tab_desc <- table1::table1(~ age_entry_years + sex + ethnicity + tdi_cats + age_education_revalued_raw + smoking + alcohol + BMI | groups, 
                           data = dat, 
...
Steps Model (with no removals)
```{r}
model_steps1 <- lm(med_steps ~ AnalysisLabel + as.factor(sex)  + age_entry_years + as.factor(season_wear) +
                      as.factor(ethnicity) + as.factor(alcohol) + tdi_raw + BMI+ as.factor(smoking),  data = dat)
summary(model_steps1)
  confint(model_steps1)
  
emmeans_model_steps_np<- emmeans(model_steps1, specs = "AnalysisLabel")
emmeans_model_steps<- emmeans(model_steps1, specs = pairwise~ AnalysisLabel)
summary(emmeans_model_steps_np)
summary(emmeans_model_steps)
confint(emmeans_model_steps)

```

```{r}
model_steps2 <- lm(med_steps ~ AnalysisLabel,  data = dat)
summary(model_steps2)
  confint(model_steps2)
  
emmeans_model_steps_np2<- emmeans(model_steps2, specs = "AnalysisLabel")
emmeans_model_steps2<- emmeans(model_steps2, specs = pairwise~ AnalysisLabel)
summary(emmeans_model_steps_np2)
summary(emmeans_model_steps2)
confint(emmeans_model_steps2)
...
Step plot 
```{r}
steps <- summary(emmeans_model_steps_np, specs = "AnalysisLabel")[1:3,c(1,2,6,5)]
...
step_plot <- ggplot()+
  ...
  scale_y_continuous(breaks = seq(0,11000, by = 1000), name = "Adjusted Mean Daily Step Count")+
  coord_cartesian(ylim = c(6500,9500))+
  xlab("")+
