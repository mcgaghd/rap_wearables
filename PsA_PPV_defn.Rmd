...
dat$date_cens[dat$data_provider== 1] <- as.Date("2017-05-31")
dat$date_cens[dat$data_provider == 2] <- as.Date("2017-03-31")
dat$date_cens[dat$data_provider == 3] <- as.Date("2016-05-31")
dat$date_cens[dat$data_provider == 4] <- as.Date("2017-08-31")
...
dat$total_prevalent_PsA_doctor <- (dat$hes_prevalent_PsA |dat$PrevalentPsA_gp)
...
##Might be the key definition
dat$ProbPsA_incident <- ((dat$hes_incident_ProbPsA & dat$total_prevalent_PsO)) 

dat$ProbPsA_incident_doctor <- (dat$hes_incident_ProbPsA & (dat$hes_prevalent_PsO |dat$PrevalentPsO_gp))


##Remove prevalent RA and AS 
dat$ProbPsA_incident <- dat$ProbPsA_incident & !((dat$total_prevalent_RA | dat$total_prevalent_AS) & !dat$total_prevalent_PsA)

##Remove prevalent RA and AS 
dat$ProbPsA_incident_doctor <- dat$ProbPsA_incident_doctor & !((dat$total_prevalent_RA | dat$total_prevalent_AS) & !dat$total_prevalent_PsA)
...
dat$ProbPsA_incident_date[!dat$ProbPsA_incident_doctor] <- NA
dat$ProbPsA_incident_date <- as.Date(dat$ProbPsA_incident_date, format = "%Y-%m-%d")
```

```{r}
ProbPsA_icd10_codes_1 <- "^M06[^3].*$"
ProbPsA_icd9_codes_1 <- "^7149$|^7165$"
...
Pull in the self-reported ever from i2 and i3 self-report 
...
```{r}
dat_i1 <- dat_i2 %>%
  filter(!is.na(date_inst_1))
```

find self-reported PsA at i1
```{r}
dat_i1$self_reported_PsA <- grepl("psoriatic arthropathy", dat_i1$conditions_at_i1)
```
...
dat_i1 <- dat_i1 %>%
  filter(self_reported_PsA != FALSE)
```

find self-reported PsA at i2
```{r}
dat_i2$self_reported_PsA <- grepl("psoriatic arthropathy", dat_i2$conditions_at_i2)
...
```{r}
dat_i1$self_reported_PsA_i1 <- dat_i1$self_reported_PsA
dat_i2$self_reported_PsA_i2 <- dat_i2$self_reported_PsA
```

Just keep relevant rows 
```{r}
dat_i1 <- dat_i1 %>%
  select(eid, self_reported_PsA_i1)

dat_i2 <- dat_i2 %>%
  select(eid, self_reported_PsA_i2)
```

```{r}
dat <- dat %>%
  left_join(dat_i1, by = "eid")

dat <- dat %>%
  left_join(dat_i2, by = "eid")
```

```{r}

dat$PsA_gp_ever <- (dat$IncidentPsA_gp | dat$PrevalentPsA_gp)
dat$PsA_HES_ever <- (dat$hes_incident_PsA | dat$hes_prevalent_PsA)

dat$PsA_doctor_ever <- dat$PsA_HES_ever |dat$PsA_gp_ever

dat$PsA_self_report_ever <-(dat$self_reported_PsA | dat$self_reported_PsA_i1 | dat$self_reported_PsA_i2)

# Convert NA to FALSE in dat$PsA_self_report_ever
dat <- dat %>%
  mutate(PsA_self_report_ever = ifelse(is.na(PsA_self_report_ever), FALSE, PsA_self_report_ever))
```
...
Censoring date for PPV defn

```{r}
dat <- dat %>%
  mutate(date_cens_1 = date_cens - years(2))
```

Participants with a recorded loss-to-follow-up date should be censored at loss-to-follow-up:
```{r}
# People who were lost to follow-up are censored at earliest of loss-to-follow-up and overall censoring
dat$date_cens_2 <- pmin(dat$date_cens_1, dat$date_lost_followup, na.rm = TRUE)
```
...
```{r}
# Convert columns to Date class if they are not already
dat$ProbPsA_incident_date <- as.Date(dat$ProbPsA_incident_date)
dat$date_cens_1 <- as.Date(dat$date_cens_1)

# Create new column ProbPsA_incident_cens
dat <- dat %>%
  mutate(ProbPsA_incident_cens = ifelse(ProbPsA_incident_date < date_cens_1, TRUE, FALSE))

dat$ProbPsA_incident_cens[is.na(dat$ProbPsA_incident_cens)] <- FALSE

#Probable with doctor PsO prevalent 
dat$ProbPsA_incident_date <- as.Date(dat$ProbPsA_incident_date)
dat$date_cens_1 <- as.Date(dat$date_cens_1)

# Create new column ProbPsA_incident_cens
dat <- dat %>%
  mutate(ProbPsA_incident_doctor_cens = ifelse(ProbPsA_incident_date < date_cens_1, TRUE, FALSE))

dat$ProbPsA_incident_doctor_cens[is.na(dat$ProbPsA_incident_doctor_cens)] <- FALSE

```

```{r}
dat$hes_incident_PsA_cens <- ifelse(!is.na(dat$PsA_incident_HES_date) & dat$PsA_incident_HES_date < dat$date_cens, TRUE, FALSE)

dat$PsA_HES_ever_cens <- dat$hes_incident_PsA_cens | dat$total_prevalent_PsA
dat$PsA_gp_ever

dat$PsA_doctor_ever_cens <- (dat$PsA_gp_ever | dat$PsA_HES_ever_cens)

```

```{r}
dat <- dat %>%
  mutate(self_reported_PsA_i2 = ifelse(is.na(self_reported_PsA_i2), FALSE, self_reported_PsA_i2))

dat <- dat %>%
  mutate(self_reported_PsA_i1 = if_else(is.na(self_reported_PsA_i1), FALSE, self_reported_PsA_i1))

dat <- dat %>%
  mutate(self_reported_PsA_i2_cens = ifelse(self_reported_PsA_i2 == TRUE & date_inst_2 < date_cens, TRUE, FALSE))

dat$PsA_self_report_total_cens <- (dat$self_reported_PsA_i2_cens | dat$self_reported_PsA | dat$self_reported_PsA_i1)
```

```{r}
dat$total_prevalent_PsO_imp <- dat$total_prevalent_PsO | dat$PsA_doctor_ever_cens

```

```{r}
dat$PsA_ever_cens <- (dat$PsA_gp_ever | dat$PsA_HES_ever_cens|dat$PsA_self_report_total_cens)
```

```{r}
library(ggvenn)

dat$ProbPsA_incident_doctor_cens_1 <-(dat$ProbPsA_incident_doctor_cens &!dat$SeropositiveRA_ever)

dat_venn <- dat

#dat_venn$'Definite PsA' <- dat_venn$PsA_ever_cens
dat_venn$'Definite PsA' <- dat_venn$hes_incident_PsA

dat_venn$'Probable PsA HES' <- dat_venn$ProbPsA_incident


plot <- ggvenn(dat_venn, c('Definite PsA', 'Probable PsA HES'),
...
dat_venn_subset <- subset(dat_venn, total_prevalent_PsO_imp == TRUE)



dat_venn_subset$'Definite PsA' <- dat_venn_subset$PsA_ever_cens
dat_venn_subset$'Probable PsA HES' <- dat_venn_subset$ProbPsA_incident


plot <- ggvenn(dat_venn_subset, c('Definite PsA', 'Probable PsA HES'),
               fill_color = c("#0073C2FF", "#EFC000FF"),
  stroke_size = 0.5, set_name_size = 4
  )
plot
...
dat$SeropositiveRA_ever <- (dat$hes_prevalent_seropositiveRA | dat$SeropositiveRA_incident)
```
