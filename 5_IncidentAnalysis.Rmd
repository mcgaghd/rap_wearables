...
primary <- c("CadencePeak30Adjusted.steps.min.")
...
```{r}
dat$step_thirds <- qtile_cut(dat[[primary]], probs = seq(0, 1, by = (1/4)))

dat$cadence1_thirds <-qtile_cut(dat$CadencePeak1Adjusted.steps.min., probs = seq(0, 1, by = (1/4)))

dat$cadence30_thirds <-qtile_cut(dat$CadencePeak30Adjusted.steps.min., probs = seq(0, 1, by = (1/4)))
  ...
  for (i in 1:length(outcomes)) {
    for (j in 1:length(adjustments)) {
      ...
      for (cat in fac_levels) {
        ...
        # Mean exposure ==================================================================================
        col_cadence <- paste0("mean_cadence_", cat)
        mean_exposure <- data.frame(mean(dat[dat[, exposure] == cat, primary]))
        colnames(mean_exposure) <- c(col_cadence)
...
my_custom_function <- function(data, xlims = c(0, 19000), ybreaks = c(0.4, 0.6, 0.8, 1, 1.2, 1.4), 
                       ...
                       save_plot = NULL) {
  required_columns <- c("mean_cadence30", "estimate", "stderr", "estlab", "nlab")
  ...
  shapeplot <- ggplot(data, aes(x = `mean_cadence30`, y = exp(estimate))) +
    ...
    xlab("Median Daily Cadence30") +
    ylab("HR for PsA Incidence")
...
for (adjustment in adjustments) {
  ...
  for (outcome in outcomes) {
    ...
    for (exposure in exposures) {
      ...
      if (exposure == "cadence30_thirds") {
        exposure_title <- "Peak cadence in 30 mins"
      } else {
      ...
      for (cat in cats){
        fp_frame[fp_frame$variable == cat, 
                 c("estimate", "stderr", "n", "n_event", "mean_cadence30")] <- 
  ...
  shapeplot1 <- my_custom_function(fp_frame_cadence30_thirds_PsA_incident, display_plot = FALSE)
  assign(paste0("shapeplot_cadence30_thirds_", gsub(",", "_", adjustment)), shapeplot1)
}
```

```{r fig.width=12, fig.height=8}
g <- grid.arrange(shapeplot_cadence30_thirds_sex + ggtitle("Minimally Adjusted"),
                  shapeplot_cadence30_thirds_sex_tdi_cats_smoking_alcohol + 
                    ggtitle("Multivariably Adjusted"), ncol=2,
                  bottom = paste0("Main analysis investigating the association",
                                  " between cadence tertiles\n and incident",
                                  " PsA"))
