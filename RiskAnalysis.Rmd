...
```{r}
temp <- survSplit(Surv(fu_time_years, RA_incident) ~ ., data = dat, cut = 3, episode="epoch")
...
model_5 <- coxph(Surv(fu_time_years, RA_incident) ~ age_entry_years+sex+BMI+RF_cont+PRS_RA, dat5,x=TRUE,y=TRUE)
summary(model_5)


# Age, sex, smoking, BMI and rheumatoid factor + Cadence30
model_6 <- coxph(Surv(fu_time_years, RA_incident) ~ age_entry_years+sex+BMI+ RF_cont+PRS_RA+CadencePeak30Adjusted.steps.min., dat5,x=TRUE,y=TRUE)
summary(model_6)
