...
```{r}
dat <- fread("GRAPPA_final.csv", data.table = FALSE) # version if running in same session as 
```
...
dat$PsAAcuteOpioid <- with(dat, total_prevalent_PsA & LongTermOpioid)
dat$PsA_NoAcuteOpioid <- with(dat, total_prevalent_PsA & !LongTermOpioid)

dat$PsOAcuteOpioid  <- with(dat, Prevalent_PsO_lessPsA & LongTermOpioid)
dat$PsO_NoAcuteOpioid  <- with(dat, Prevalent_PsO_lessPsA & !LongTermOpioid)
