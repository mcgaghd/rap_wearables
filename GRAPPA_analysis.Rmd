...
```{r}
datPsA <- subset(dat, total_prevalent_PsA == TRUE)
datPsO <- subset(dat, Prevalent_PsO_lessPsA == TRUE)
```

```{r}
model_steps1 <- lm(CadencePeak30Adjusted.steps.min. ~ SystemicCorticosteroids + as.factor(sex)  + as.factor(age_gp) + as.factor(tdi_cats),  data = datPsA)
...
#Set up a corresponding label to these categories 
labels3 <- c("HC", "PsO + no opioid", "PsO + persistent opioid", "PsA + no opioid", "PsA + persistent opioid")
...
#Set up a corresponding label to these categories 
labels3 <- c("HC2", "PsO + no opioid", "PsO + acute opioid", "PsA + no opioid", "PsA + acute opioid")
...
#Set up a corresponding label to these categories 
labels3 <- c("HC", "PsO + no steroids", "PsO + steroids", "PsA + no steroids", "PsA + steroids")
...
```{r}
steps <- summary(emmeans_model_steps1_np, specs = "PsoriaticSteroids")[1:5, c(1, 2, 6, 5)]
names(steps) <- c("Quality", "Mean", "UpperCI", "LowerCI")
steps$Quality <- c("None","PsO+no steroids", "PsO+steroids", "PsA+no steroids ","PsA+steroids")
steps$Quality <- factor(steps$Quality, levels = c("None","PsO+no steroids", "PsO+steroids", "PsA+no steroids ","PsA+steroids"))
